# Soundness investigation for the original λp calculus
...
`LambdaP/Original/SourceUnsoundnessCounterexample.lean`; the summary theorem is
...
nested binder.  All files under `LambdaP/Original` are checked without
...
1. **Reconstruction and binding lemmas.**  The pre-restart syntax, contexts,
   ...
   `LambdaP.Original`.  Renaming, weakening, simultaneous path substitution,
...
in `LambdaP.Repaired`, and here it is justified by a concrete source
...
`SourceCounterexampleBlocked.lean` show that the crucial final edges of the
...
`LambdaP.Repaired` development, which implements the syntax split.

## Soundness proof for the repaired calculus
...
two operations.  `PairRuleExamples.lean` checks that this is still enough to
...
`StructuralMappedRealization.lean` defines a proof-relevant logical relation.
...
The final source-facing results are in `StructuralClosedSafety.lean`:
...
sufficient for the standard non-stuckness theorem.  The complete repaired
