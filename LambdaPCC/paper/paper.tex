...
types, shape types, and member signatures. For each category it is the
...
\]
Conversion preserves the outer constructor and the form of a member signature. Beneath a
...
\infrule[\ruledef{ke-hole}]
  {\Omega;\sigma\vdash U\tcoerce T
   \\
   \Omega;\sigma\vdash D\semleq C}
  {\Omega;\sigma\vdash[\,]:(U,D)\Rightarrow(T,C)}
\end{minipage}\hfill
...
For a direct value, the conclusion follows from value typing and the type
coercion retained by the empty continuation. For a returned location,
annotated store typing supplies the value and its assigned capture set;
\ruleref{cap-fold} and the type coercions retained by path and continuation
typing relate that set to \(C'\).
