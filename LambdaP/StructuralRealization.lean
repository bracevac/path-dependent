...
/-!
Store-indexed realization for the calculus.
...
The distinction between term and type paths appears in the last two
proper-type cases: `single` is realized by co-resolution with a term
location, whereas `tsel` is realized through a stored type definition.
-/
