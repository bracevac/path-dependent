...
/-!
Regression for the closed source counterexample against the original calculus.

The repaired syntax distinguishes the abstract selection `q.A` from the
...
namespace LambdaP.Repaired.SourceCounterexampleBlocked

open LambdaP.Repaired
...
/-- Every repaired subtyping rule preserves the interpretation.  The proof
...
| z : KnownPathTy (Path.var SourceCounterexampleBlocked.z)
    (Tau.ty argumentType)
| q : KnownPathTy (Path.var SourceCounterexampleBlocked.q)
    (Tau.ty qType)
| q_fst : KnownPathTy (Path.var SourceCounterexampleBlocked.q).fst
    (Tau.ty Ty.Top)
...
end LambdaP.Repaired.SourceCounterexampleBlocked
