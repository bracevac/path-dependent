...
/-!
The small-step CK machine from the repaired development.  Paths are resolved
by the literal historical `Path.reduce` relation; the machine itself retains
only term-level `let` frames.
-/

namespace LambdaP.Repaired

/-- One transition of the original indexed machine. -/
