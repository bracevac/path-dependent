...
theorem Ty.weaken_subst_lift
    (T : LambdaP.Repaired.Ty n) (rho : PathSubst n m) :
    T.weaken.subst rho.lift = (T.subst rho).weaken := by
  simp only [LambdaP.Repaired.Ty.weaken, Ty.rename_subst,
    Ty.subst_rename]
  rfl
...
theorem Path.StructSubstitution.openAt
    {n : Nat} {Gamma : Ctx n} {R : Path n -> Path n -> Prop}
    {q : Path n} {S : LambdaP.Repaired.Ty n}
  ...
  | here =>
      simpa only [PathSubst.openAt_zero, Tau.subst,
        LambdaP.Repaired.Ty.weaken_subst_openAt] using hq
  | there hx =>
      simpa only [PathSubst.openAt_succ, Tau.subst,
        LambdaP.Repaired.Ty.weaken_subst_openAt] using
...
theorem Tau.StructWf.open_path
    {n : Nat} {Gamma : Ctx n} {R : Path n -> Path n -> Prop}
    {S : LambdaP.Repaired.Ty n} {q : Path n} {k : Kind}
    {d : Tau (n + 1) k}
