import LambdaP.Repaired.StructuralPreciseProgress
import LambdaP.Repaired.StructuralPrecisePreservation
...
inductive State.PreciseStructSafetyOutcome
    ...
    (T : LambdaP.Repaired.Ty n) : Prop where
| final :
    source.IsFinal ->
...
inductive State.PreciseStructExtension :
    {n m : Nat} -> Ctx n -> LambdaP.Repaired.Ty n ->
      Ctx m -> LambdaP.Repaired.Ty m -> Prop where
| refl : State.PreciseStructExtension Gamma T Gamma T
| snoc {n m : Nat} {Gamma : Ctx n} {T : LambdaP.Repaired.Ty n}
    {Delta : Ctx m} {U : LambdaP.Repaired.Ty m} :
    State.PreciseStructExtension Gamma T Delta U ->
    (S : LambdaP.Repaired.Ty m) ->
