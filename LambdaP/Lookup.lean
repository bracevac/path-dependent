...
/-!
A variant of the source big-step path semantics whose miss rule follows the
same prefix that appears in the static selection rule. The machine semantics
recurses from `Path.var y`, where `y` is obtained by looking up the
...
/-- In the source semantics, selection depends only on the location of
...
/-! ## Equivalence with the source semantics -/
...
/-- The static-aligned and machine miss rules define the same lookup
