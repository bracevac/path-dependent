...
theorem Tau.Sub.mayPairHead
  ...
  | sel_hi hp _ _ =>
      ...
      | single_intv hp' hh' =>
          ...
          exact hh'
  | sel_lo hp _ ih =>
      exact Tau.MayPairHead.single_intv hp (ih hh)
  | «fun» _ _ _ _ =>
      cases hh
  | pair _ _ _ _ =>
      cases hh
...
theorem Tau.Sub.mayFstHead
  ...
  | sel_hi hp _ _ =>
      ...
      | single_intv hp' hh' =>
          ...
          exact hh'
  | sel_lo hp _ ih =>
      exact Tau.MayFstHead.single_intv hp (ih hh)
...
theorem Tau.Sub.mayFstPairHead
  ...
  | sel_hi hp _ _ =>
      ...
      | single_intv hp' hh' =>
          ...
          exact hh'
  | sel_lo hp _ ih =>
      exact Tau.MayFstPairHead.single_intv hp (ih hh)
  | «fun» _ _ _ _ => cases hh
  | pair hfst _ _ _ =>
      cases hh with
...
theorem Tm.PreciseTy.pair_canonical_kind
    {n : Nat} {Γ : Ctx n} {v : Tm n} {P S : LambdaP.Original.Ty n}
...
def Path.PairTransport (Γ : Ctx n) (σ : Store n) : Prop :=
  ∀ {p : Path n} {x : Fin n} {S : LambdaP.Original.Ty n} {a : Name}
      {k : Kind} {d : Tau (n + 1) k},
    Path.reduce p σ x ->
...
theorem Store.RefinedTy.variable_pair_transport
    ...
    {S : LambdaP.Original.Ty n} {a : Name} {k : Kind}
    {d : Tau (n + 1) k}
    (hσ : Store.RefinedTy Γ σ)
...
/-!
### Remaining dependent-member boundary

`mayFstHead` and `mayFstPairHead` show that ordinary source subtyping loses
no elimination information about a pair's first component.  The analogous
member proof is not a plain source-subtyping lemma.  In the `Tau.Sub.pair`
case the member premise lives under the *source* context `Γ.snoc S`, whereas
selection observes the target member after opening it with `p.fst` in `Γ`.
For a concrete runtime pair the source member is opened with `var y`, and
only runtime co-resolution relates `p.fst` and `var y`.

Thus the exact remaining contract is an observation-level, runtime-aware
opening lemma: a member observation transported by
`Tau.Sub (Γ.snoc S) d₁ d₂` must remain valid after opening `d₁` and `d₂` by
runtime-equivalent paths.  The existing `Tau.Sub.open_var` handles only one
exactly typed variable and cannot establish this two-path statement.  Until
that scoped opening result is available, the unconditional composite-path
instance of `Path.PairTransport` does not follow from `Store.RefinedTy`.
-/
