...
inductive Preserve : Ctx n -> State m -> LambdaP.Original.Ty n -> Prop where
| same :
    State.Ty Γ s T ->
    Preserve Γ s T
| extend :
...
theorem Tm.Ty.let_inv_of_eq
    {n : Nat} {Γ : Ctx n} {u : Tm n} {T : LambdaP.Original.Ty n}
    (h : Tm.Ty Γ u T) :
...
theorem Tm.Ty.typed_inv_of_eq
    {n : Nat} {Γ : Ctx n} {u : Tm n} {T : LambdaP.Original.Ty n}
    (h : Tm.Ty Γ u T) :
    ∀ {t : Tm n} {A : LambdaP.Original.Ty n}, u = Tm.typed t A ->
