...
theorem Ty.weaken_subst_lift
    (T : LambdaPHistory.Ty n) (rho : PathSubst n m) :
    T.weaken.subst rho.lift = (T.subst rho).weaken := by
  simp only [LambdaPHistory.Ty.weaken, Ty.rename_subst,
    Ty.subst_rename]
  rfl
...
theorem Path.StructSubstitution.openAt
    {n : Nat} {Gamma : Ctx n} {R : Path n -> Path n -> Prop}
    {q : Path n} {S : LambdaPHistory.Ty n}
  ...
  | here =>
      simpa only [PathSubst.openAt_zero, Tau.subst,
        LambdaPHistory.Ty.weaken_subst_openAt] using hq
  | there hx =>
      simpa only [PathSubst.openAt_succ, Tau.subst,
        LambdaPHistory.Ty.weaken_subst_openAt] using
...
theorem Tau.StructWf.open_path
    {n : Nat} {Gamma : Ctx n} {R : Path n -> Path n -> Prop}
    {S : LambdaPHistory.Ty n} {q : Path n} {k : Kind}
    {d : Tau (n + 1) k}
