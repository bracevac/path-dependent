...
/-!
The remaining application obligation, factored at the point where the
...
def Store.StructFunctionReflection (Gamma : Ctx n) (sigma : Store n) : Prop :=
  forall {q : Path n} {x y : Fin n}
      {S A X : LambdaPHistory.Ty n}
      {U B : LambdaPHistory.Ty (n + 1)}
      {body : Tm (n + 1)},
...
theorem Tm.StructCheck.cast_open_result_runtime
    ...
    {q : Path n} {y : Fin n} {U : LambdaPHistory.Ty (n + 1)}
    {t : Tm n}
    (hq : Path.reduce q sigma y)
...
Proving this from `Store.StructTy` is the remaining public-widening/canonical
forms problem.  Abstract members are the non-syntax-directed case: their
...
def Store.StructResultOpening (Gamma : Ctx n) (sigma : Store n) : Prop :=
  forall {q : Path n} {y : Fin n} {S : LambdaPHistory.Ty n}
      {B U : LambdaPHistory.Ty (n + 1)} {t : Tm n},
    Path.reduce q sigma y ->
    Tm.StructCheck Gamma (Path.RuntimeEq sigma) (Tm.path q) S ->
