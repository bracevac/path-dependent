...
inductive State.StructSafetyOutcome
    ...
    (T : LambdaPHistory.Ty n) : Prop where
...
inductive State.StructExtension :
    {n m : Nat} -> Ctx n -> LambdaPHistory.Ty n ->
      Ctx m -> LambdaPHistory.Ty m -> Prop where
| refl : State.StructExtension Gamma T Gamma T
| snoc {n m : Nat} {Gamma : Ctx n} {T : LambdaPHistory.Ty n}
    {Delta : Ctx m} {U : LambdaPHistory.Ty m} :
    State.StructExtension Gamma T Delta U ->
    (S : LambdaPHistory.Ty m) ->
