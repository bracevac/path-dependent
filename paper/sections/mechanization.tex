...
\paragraph{Axioms.}
The theorems of Sections~\ref{sec:canonical} and~\ref{sec:translation} depend on propositional extensionality and quotient soundness only, with two exceptions: progress and the backward erasure simulation use choice to pick a step, and safety of \DOTMNF inherits it. The tree contains no unproven assumption, no admitted lemma, and no unbounded recursion. The pigeonhole argument behind resolution is proven from scratch, because the core library has no lemma for it and the nearby list lemmas depend on choice.
