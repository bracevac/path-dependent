...
either a term member or a type member. The member signature may mention the
first component: term members have proper-type signatures, while type members
have interval signatures.

The central subtyping rule is covariant in both pair components and compares
the member signatures under the source first-component type. Its dependent
premise cannot be interpreted until a concrete pair realization supplies the
location bound by that premise. The soundness proof interprets each subtyping
derivation as a store-indexed coercion. A path resolves to a referent, which
is either a location or a stored type definition, and the realization
judgment records which signatures that referent supports. Applying a
coercion transforms a realization derivation for its source signature into one
for its target signature.

Dependent premises for function codomains and pair members are retained with
their closing substitutions until the corresponding location is known. In
the pair case, the member premise is interpreted at the stored first
component and the resulting coercion is applied to the member referent.
Coercion application is well founded by lexicographic induction on
allocation rank and coercion size: both referents contained in a pair precede
the pair cell, while a stored type definition has rank zero. We give the
complete calculus and operational semantics, the coercion interpretation,
the machine-typing invariant, and proofs of progress and preservation. The
development is mechanized in Lean 4.
\end{abstract}
...
transitivity rule, and bound-consistency premises for type intervals.
...
Applying the pair coercion at a location \(z\) exposes a stored first
component \(y\) and member definition \(\delta\). The first coercion
establishes that \(y\) realizes \(S'\). The suspended member premise extends
its closing substitution by \(x\mapsto y\), produces a coercion from
\(\tau[y/x]\) to \(\tau'[y/x]\), and applies that coercion to the referent of
\(\delta\). This last coercion need not occur within the enclosing pair
coercion. Its application nevertheless decreases the allocation
rank,
because \(y\) and every location mentioned by \(\delta\) precede \(z\) in
the immutable store.
...
types, while pair introduction exposes exact information: a term member
...
\infrule[\ruledef{wf-select}]
  {\Gamma\vdash p\Rightarrow\pairty{x}{S}{A}{T..U}}
  {\Gamma\vdash p.A\ \wf}
\end{minipage}\hfill
\begin{minipage}[t]{0.51\linewidth}
...
rules, and all seven signature well-formedness rules.}
...
subtyping.}
\Description{All eleven signature-subtyping rules.}
\label{fig:subtyping}
\end{figure}
...
The proof has three stages. First, typed path resolution interprets a
path-synthesis derivation as a run-time resolution derivation together with a
realization derivation for its referent. Second, the subtyping interpretation
maps a subtyping derivation to a store-indexed coercion, and coercion
application maps source realizations to target realizations. An open
codomain coercion retains a dependent function premise until application
supplies the argument location; a suspended member premise retains the
dependent pair premise until inversion of pair realization supplies its
first-component location.
Third, interpretation of source typing produces syntax-directed machine
typing for the current term. Continuation typing records how the current
result is used, and the two judgments form the invariant for progress and
preservation. Sections~\ref{sec:coercions} and~\ref{sec:invariant} define
these interpretations before applying them to the soundness proof.

\section{Store-indexed semantic interpretation}
\label{sec:realization}
\label{sec:coercions}

\subsection{Store-induced equality}
...
of paths that resolve to the same referent and closed under substitution into
paths. Store-induced signature equality
\(\tau\simeq_\sigma\tau'\) is the least sort-preserving equivalence generated
by replacement of store-equal paths:
\[
  p\equiv_\sigma q
  \quad\Longrightarrow\quad
  \subst{\tau_0}{p}{z}\simeq_\sigma\subst{\tau_0}{q}{z}.
\]
The analogous substitution rule makes path equality a congruence. Both
substitutions replace every free occurrence of \(z\), alpha-renaming an
intervening binder when necessary.
...
signature relation \(\simeq_{\sigma;z}\) is generated by replacement with
...
\cref{fig:semantic-judgments}; scoped equality uses the same rules with the
rigid path relation.

\subsection{Realization and coercions}

Fix a source context \(\Gamma\) and a store \(\sigma\). A closing
substitution has signature
\[
...
\(\rho(X)\) denotes capture-avoiding application of the closing substitution,
which replaces each free source variable \(x\) by \(\rho(x)\). The
substitution extends homomorphically to syntax; for paths,
\[
\rho(p.1)=\rho(p).1,
\qquad
\rho(p.a)=\rho(p).a.
...
\(z\notin\mathsf{dom}(\Gamma)\) and \(y\in\mathsf{dom}(\sigma)\), extension
of the closing substitution has signature
\[
  \rho[z\mapsto y]:
...
\(\sigma\vdash q\resolve r\). Store-induced signature equality changes only
...
\plateheader{Store-induced signature equality}{\tau\simeq_\sigma\tau'}
\typicallabel{seq-replace}
\infax[\ruledef{seq-refl}]
  {\tau\simeq_\sigma\tau}

\infrule[\ruledef{seq-symm}]
  {\tau\simeq_\sigma\tau'}
  {\tau'\simeq_\sigma\tau}

\infrule[\ruledef{seq-trans}]
...
\end{minipage}
\caption{Store-indexed judgments (part 1): path and signature equality,
context satisfaction, suspended body typing, and realization.}
\Description{All five path-equality rules, all four signature-equality rules,
the context rule, the suspended-body rule, both referent rules, and all five
location-realization rules.}
...
\begin{figure}[H]
\ContinuedFloat
\setlength{\afterruleskip}{1.55em plus 0.15em minus 0.1em}
...
\plateheader{Suspended subtyping derivation}{}
...
\end{minipage}
\caption{Store-indexed judgments (part 2): suspended member premises,
semantic coercions, and open codomain coercions.}
\Description{The suspended-member rule, all twelve semantic-coercion rules,
and all five open-codomain rules.}
\end{figure}
...
realization rule. Rule \ruleref{loc-fun} records the stored abstraction and its
...
The equality coercion embeds store-induced signature equality, whose
soundness follows from realization invariance below.
...
The dependent cases suspend their binder premises. An open codomain coercion
may be composed, transported by store-induced equality, or narrowed before
the preservation proof supplies an argument location. As specified by
\ruleref{mem-source}, a suspended member premise pairs an open member-subtyping
derivation with a closing substitution satisfying its outer context. It is
interpreted when inversion of pair realization supplies the stored first
component. The two forms differ because open
codomain coercions must survive further function subtyping, whereas the member
premise is interpreted immediately after pair-realization inversion.

\begin{lemma}[Constructor inversion for store-induced equality]
...
\end{array}
\]
In particular, \(\top\) and \(\bot\) are equal only to themselves, and a
proper type is never equal to an interval.
\end{lemma}

The proof is induction on signature equality. Replacement preserves the
outer constructor by induction on the signature \(\tau_0\) in the replacement
rule. In the
transitivity case, both induction hypotheses identify the constructor of the
intermediate signature, and their component equalities compose.
...
of resolution under path equality. Function domains use equality in the
...
with the scoped codomain equality. In the pair
case, scoped equality of member
...
coercions induced by their component equalities.
...
Typed resolution uses realization invariance under store-induced equality;
subtyping interpretation then uses typed resolution. Member instantiation
interprets a suspended premise under an extended closing substitution, and
coercion application invokes it in the pair case. The preservation argument
instantiates an open codomain coercion after a function application identifies
...
store-induced equality and \cref{lem:conversion-transport} transfer the
...
\raggedright
\rulesetup
\setlength{\afterruleskip}{1.35em plus 0.15em minus 0.1em}
...
The selection cases use the lower or upper coercion already present in the
realized interval. The dependent cases retain \(\pi_2\) without recursively
interpreting it. Its closing substitution lacks the bound variable until
an argument location is known or pair-realization inversion supplies the
first-component location.
\endgroup
\caption{Interpretation of all eleven subtyping rules as semantic coercions.}
\Description{The eleven equations defining subtyping interpretation. The
function and pair coercions retain their dependent premise derivations.}
...
The pair case is the only application clause that interprets a suspended
source derivation. It is stated in full below.
...
Instantiation preserves identity, composition, and store-induced equality.
...
The transitivity and selection cases recurse on coercions appearing as
premises of the current coercion derivation. The pair case also invokes the coercion obtained by
...
\ruleref{c-sel-lo}, and \ruleref{c-sel-hi} retain the referent and recurse on
a structurally smaller coercion, decreasing the secondary component. In
...
instantiation. These are all recursive coercion applications.

\subsection{Semantic subtyping}
\label{sec:coercion-totality}

The preceding definitions have the following typing properties.
...
derivation. The well-founded induction needed for the pair case was established in
...
current term to the final result type. The empty-continuation rule permits a
coercion, absorbing subsumption at the boundary of the machine stack.
...
\ruleref{kont-hole} at reflexivity and \ruleref{cfg-state}, gives
\(\vdash\mathsf{initial}(t):T\) from \(\ctxempty\vdash t:T\).

This interpretation connects the semantic coercions to preservation. Source
subsumption becomes the final coercion in syntax-directed machine typing.
Elimination cases apply that coercion to obtain the realization required for
inversion, such as \ruleref{loc-fun} in the application case. Allocation
constructs a realization at the type assigned by the syntax-directed value
rule and then applies the final coercion. Body and codomain premises are instantiated after a
reduction identifies their bound location; a member premise is instantiated
during coercion application after pair-realization inversion.
...
resolution, store-induced path and signature equality, and all store-indexed
semantic and machine-typing judgments over \(\sigma\) remain valid over
\(\sigma'\).
\end{lemma}
...
\item Since \(q\) and the location name \(y\) resolve to the same location,
  store-induced signature equality gives a coercion \(C_q\) from
...
The formalized results cover the complete calculus and machine, typed path
resolution, subtyping interpretation, coercion application, machine typing,
progress, preservation, and \cref{thm:safety}.
...
The dependent-pair covariance rule is sound when its member premise remains
available until inversion of a concrete pair realization supplies the
first-component location.
Store-indexed semantic coercions provide the required proof object. They
compose along transitivity, retain the resolution derivations and lower- or
upper-bound coercions needed by singleton and selection rules, and transform realizations of referents
without extending the program syntax. Pair coercion application instantiates
its suspended member premise at the stored component and terminates by
descending in the allocation-order rank.

This construction proves progress and preservation for \lambdap{} with
arbitrary paths, abstract type members with interval signatures, an explicit
transitivity rule, and covariance of both pair components. The proof relies on
typed resolution, realization at referents, derivation-directed subtyping
interpretation, and syntax-directed machine typing with final coercions at
the typing boundary.
