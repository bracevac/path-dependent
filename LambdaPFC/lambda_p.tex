...
The soundness proof interprets each subtyping derivation as a store-indexed
coercion that transforms realization derivations for its source signature
into realization derivations for its target signature. Dependent premises for
function codomains and pair members are retained with their closing
substitutions until the corresponding location is known, and coercion
application is well founded by lexicographic induction on allocation rank and
coercion size. We give the complete calculus and operational semantics, the
coercion interpretation, the machine-typing invariant, and proofs of progress
and preservation, all mechanized in Lean 4.
\end{abstract}
...
Applying the pair coercion at a location instantiates the suspended premise
...
not structurally contained in the pair coercion, its application decreases
...
The proof has three stages: typed path resolution interprets path-synthesis
derivations as run-time resolutions together with realizations of their
referents; the subtyping interpretation compiles subtyping derivations to
store-indexed coercions, whose application maps source realizations to
target realizations; and interpretation of source typing produces the
syntax-directed machine invariant for progress and preservation
(\cref{sec:invariant}).
...
composed, transported by store-induced equality, or narrowed before the
...
The dependent cases retain \(\pi_2\) without recursively interpreting it.
\endgroup
...
the extended store leaves existing names unchanged. In the intrinsically
scoped mechanization, allocation renames every location, including those in
the result type: the Lean preservation theorems return typing at a weakened
copy of \(T\) (the relation \texttt{Ty.Extends}), which the named
presentation renders invisible.
...
The named presentation and the Lean development correspond as follows.
\begin{center}
\small
\begin{tabular}{ll}
\toprule
Paper & Lean\\
\midrule
closing substitution \(\rho\) & \texttt{Valuation}\\
context satisfaction \(\rho\realizes_\sigma\Gamma\) & \texttt{Environment}\\
realization \(\sigma\realizes r:\tau\) &
  \texttt{Store.Possible}, \texttt{Path.Referent.Realizes}\\
coercions and suspended premises &
  \texttt{Coercion}, \texttt{DeferredCoercion},\\
& \texttt{MemberClosure}, \texttt{BodyClosure}\\
typed resolution (\cref{lem:typed-resolution}) & \texttt{Path.Ty.resolve}\\
subtyping interpretation (\cref{lem:coercion-construction}) &
  \texttt{Tau.Sub.compile}\\
coercion application (\cref{lem:coercion-action}) &
  \texttt{Coercion.action}\\
allocation rank and \(|C|\) & \texttt{stratum}, \texttt{treeSize}\\
machine typing (\cref{fig:machine-typing}) &
  \texttt{ValueEvidence}, \texttt{TermEvidence}, \texttt{State.Evidence}\\
\cref{thm:progress} & \texttt{State.Evidence.progress}\\
\cref{thm:preservation} &
  \texttt{State.Evidence.preservation} (via \texttt{Ty.Extends})\\
\cref{thm:safety} & \texttt{Tm.Ty.closed\_type\_safety}\\
\bottomrule
\end{tabular}
\end{center}
Two rule names differ: \ruleref{s-alias} is the constructor \texttt{symm},
and \ruleref{c-eq} is \texttt{runtime}.
...
\citep{parreaux2019gadt,boruchgruszecki2022cdot}, and type-preserving
translations from class-based calculi to DOT connect a source language to a
sound path-dependent core \citep{martres2023compilation}. Both address
features absent from \lambdap{}.
...
first-component location. Store-indexed semantic coercions provide the
required proof object, and the allocation order of the immutable store makes
their application well founded. This construction proves progress and
preservation for \lambdap{} with arbitrary paths, abstract type members with
interval signatures, an explicit transitivity rule, and covariance of both
pair components.
