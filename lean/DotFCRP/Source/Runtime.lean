...
first or collapse directly to its final transparent target.  The M6 bridge
uses anchor coherence and target stuttering, not source-step determinism.
-/
...
/-- Embed the erased M5 runtime into the path runtime. -/
...
/-- Erasure commutes with the complete M5 syntax embedding. -/
@[simp]
...
/-- The embedded M5 mutual object still erases to unit. -/
