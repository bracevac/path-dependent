...
/-!
The operational semantics of `lambda_p`, stated over the native `LambdaPCCI`
...
`Path.Resolve` follows a path to a location, a stored type definition, or a
stored capture-set definition.  The term machine uses the location case
directly; type and capture referents are inspected only by the static and
semantic judgments.
-/
