...
/-!
A capture-aware mirror of the full abstract type-member interval merge.
...
The value enters the union lower bound, crosses the selected type, and exits
through both projections of the intersection upper bound.  Every exposed
capture and use set is empty.
-/

namespace LambdaPCCI.TypeMemberUnionRegression
...
def leftRecord (n : Nat) : Ty n :=
  .capt .empty
    (recordShape .Bot (IntersectionRegression.leftViewShape (n + 1)))
...
def mergedLowerShape (n : Nat) : Shape n :=
  .Union .Bot (IntersectionRegression.sourceShape n)

def mergedRecord (n : Nat) : Ty n :=
  .capt .empty
    (recordShape
      (mergedLowerShape (n + 1))
...
private def mergedLowerShapeWf {Gamma : Ctx n} :
    Shape.Wf Gamma (mergedLowerShape n) :=
  .union .bot sourceShapeWf

private def mergedBounds {Gamma : Ctx n} :
    Shape.Sub Gamma (mergedLowerShape n)
      (IntersectionRegression.intersectionShape n) :=
  .union
    (.inter .bot .bot)
    sourceShapeToIntersection
...
private def leftRecordWf {Gamma : Ctx n} :
  ...
  .capt .empty
    (.pair pureTopWf (.type .bot leftViewShapeWf .bot))
...
private def mergedRecordWf {Gamma : Ctx n} :
  ...
  .capt .empty
    (.pair pureTopWf
      (.type mergedLowerShapeWf intersectionShapeWf mergedBounds))
...
private def sourceShapeToLeftRecordShape {Gamma : Ctx n} :
  ...
  .pair .refl
    (.type .bot IntersectionRegression.sourceToLeft .refl)
...
private def recordIntersectionToMerged {Gamma : Ctx n} :
    Ty.Sub Gamma (recordIntersection n) (mergedRecord n) := by
  exact .capt .refl .pair_type_union_inter
...
/-- A single empty-use alias at the fully merged record type. -/
...
private def mergedRecordPathTyping :
  ...
  simpa [context3, context2, context1, Ctx.lookup, mergedRecord, recordShape,
    mergedLowerShape, IntersectionRegression.sourceShape,
...
private def selectedTypePathTyping :
  ...
  simpa [mergedRecord, recordShape, mergedLowerShape, Tau.open, Ty.open,
    Shape.open, Tau.subst, Ty.subst, Shape.subst, CaptureSet.subst,
    Path.subst, PathSubst.openAt, IntersectionRegression.sourceShape,
    IntersectionRegression.intersectionShape,
    IntersectionRegression.leftViewShape,
    IntersectionRegression.rightViewShape,
    IntersectionRegression.functionType,
...
private def selectedTypeWf :
  ...
  .capt .empty (.select selectedTypePathTyping mergedBounds)
  ...
  .sub
    ...
    (.capt
      ...
      (.trans
        ...
        (.trans
          .union_right
          (.select_lower selectedTypePathTyping mergedBounds))))
    (.path valuePathTyping)
    selectedTypeWf
    .empty
...
def functionViewTyping :
  ...
  .sub
    ...
    (.capt .refl
      (.trans
        (.select_upper selectedTypePathTyping mergedBounds)
        .inter_left))
    .refl
...
end LambdaPCCI.TypeMemberUnionRegression
