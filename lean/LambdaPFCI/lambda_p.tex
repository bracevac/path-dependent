...
rules. An intersection records two semantic realizations of one location; it
adds no value form, cast, or machine transition.
...
location is known. In the pair case, inversion supplies the stored first
...
\item a small calculus combining arbitrary stable paths, singleton types,
  ...
  dependent pairs with term and type members;
...
The binary proper type \(\interty{S}{T}\) is deliberately only a meet. Its
subtyping rules introduce a meet from two views of the same source and
eliminate either component. There is no term-level intersection
introduction: existing subsumption is the only way to assign an intersection
type to a term. In particular, path synthesis is not extended with rules
that project through intersections, and the calculus has no member merging,
distributivity rules, or intersection-specific record operation.
...
Binary intersections use exactly the meet rules: a source below both
components is below their intersection, and an intersection is below either
component. These rules can combine otherwise independent views of one path,
but they do not change the precise type synthesized by \(\mathsf{Path.Ty}\)
(the judgment written \(\Gamma\vdash p\Rightarrow\tau\) here).
...
are unchanged by the meet extension.
...
subtyping.}
\Description{All fourteen signature-subtyping rules.}
\label{fig:subtyping}
\end{figure}
...
rule, all fifteen semantic-coercion rules, and all five open-codomain rules.}
\end{figure}
...
\endgroup
\caption{Interpretation of all fourteen subtyping rules as semantic coercions.}
\Description{The fourteen equations defining subtyping interpretation. The
...
coercions appearing as premises of the current coercion derivation. The pair case also invokes the coercion obtained by
...
locations and leaves this renaming implicit. It also includes a closed
regression theorem exercising intersection introduction and both
eliminations in one application.
...
and studies paths built from immutable dependent pairs. It retains only
controlled binary intersections with the three meet rules: unlike richer DOT
intersections, they do not merge members, distribute through type
constructors, or participate in precise path synthesis. The resulting runtime makes allocation order available as a
...
The dependent-pair case retains the member-subtyping premise until inversion
...
elimination. It has no path-typing projections, member merging,
distributivity, term introduction form, or operational rule; extending any of
those dimensions would be a separate calculus change.
