...
meet and join rules, plus controlled rules that merge aligned pair views.
One rule intersects differing first-component views when the label and member
signature agree. The remaining rules intersect proper term-member signatures;
for intervals, one preserves a shared lower bound and one joins arbitrary
lower bounds, while both intersect their upper bounds.
...
\item a small calculus combining arbitrary stable paths, singleton types,
  ...
  controlled merging of aligned first-component views, same-label term-member
  views, and both shared-lower and general type-member interval views;
...
Four additional rules merge aligned pair views. The first combines two views
whose label and complete member signature agree, intersecting only their
first-component types:
\[
  \interty{\pairty{x}{S}{a}{\tau}}{\pairty{x}{T}{a}{\tau}}
  <:\pairty{x}{\interty{S}{T}}{a}{\tau}.
\]
The converse is already derivable: pair covariance projects
\(\interty{S}{T}\) to \(S\) and \(T\), and meet introduction combines the
two resulting pair views. Thus this restricted shape commutes with
intersection up to mutual subtyping. For proper term-member signatures with
an identical first component, the member types are intersected:
\[
...
or unions, and the calculus has no merging for unequal labels, simultaneous
changes to both pair components, unrestricted distributivity, or
meet/join-specific record operation.
...
meeting the upper bounds. Rule \ruleref{s-pair-first-inter} instead combines
differing first-component types when the label and entire member signature
agree. Thus none of the merge rules combines differing labels or changes both
pair components simultaneously. None changes the precise type synthesized by
\(\mathsf{Path.Ty}\) (the judgment written
\(\Gamma\vdash p\Rightarrow\tau\) here).
As with subtyping generally, the general interval merge rule does not assert
that its target is well formed. Any use of that target in term subsumption
supplies the separate \ruleref{wf-bounds} derivation for the joined lower and
intersected upper.
...
\infax[\ruledef{s-pair-first-inter}]
  {\begin{gathered}
   \Gamma\vdash
    \interty{\pairty{x}{S}{a}{\tau}}{\pairty{x}{T}{a}{\tau}}
   \\
...
subtyping.}
\Description{All twenty-one signature-subtyping rules.}
\label{fig:subtyping}
\end{figure}
...
\noindent
\begin{minipage}[t]{0.48\linewidth}
\typicallabel{c-pair-first-inter}
\infax[\ruledef{c-pair-first-inter}]
  {\begin{gathered}
   \sigma\vdash
    \interty{\pairty{z}{S}{a}{\tau}}{\pairty{z}{T}{a}{\tau}}
   \\
   \coerce\pairty{z}{\interty{S}{T}}{a}{\tau}
   \end{gathered}}
\end{minipage}\hfill
\begin{minipage}[t]{0.50\linewidth}
...
rule, all twenty-two semantic-coercion rules, and all five open-codomain rules.}
\end{figure}
...
Rules \ruleref{c-pair-first-inter}, \ruleref{c-pair-inter},
\ruleref{c-pair-type-inter}, and \ruleref{c-pair-type-union-inter} are
likewise nullary. Their actions invert the two pair realizations stored by
\ruleref{loc-inter} and use functionality of store lookup to identify their
one stored pair. The first combines the two realizations of that pair's first
location and reuses its identical member realization. The term-member rule
instead combines two realizations of the member location. The restricted
type-member rule identifies one stored type witness, reuses a coercion from
the shared lower bound to that witness, and intersects the two coercions from
the witness to the advertised upper bounds. The general rule combines both
lower coercions by union elimination and both upper coercions by intersection
introduction.
...
\typicallabel{s-pair-first-inter}
\infax[\ruleuse{s-pair-first-inter}]
  {\begin{gathered}
   \compilepi{\mathcal R}{\pi}=\\[-1pt]
   \ruleuse{c-pair-first-inter}
   \end{gathered}}
...
\endgroup
\caption{Interpretation of all twenty-one subtyping rules as semantic coercions.}
\Description{The twenty-one equations defining subtyping interpretation. The
...
The aligned first-component merge also acts only on evidence. Suppose the two
components of an intersection realization invert to
\[
\begin{array}{rcl}
  R_L&=&\ruleuse{loc-pair}(B_x,R_S,R_\tau),\\[2pt]
  R_R&=&\ruleuse{loc-pair}(B_x',R_T,R_\tau').
\end{array}
\]
Both derivations describe the pair stored at the same receiver location.
Functionality of immutable-store lookup identifies their stored first
location, label, and member definition. Because the source rule requires an
identical member signature, either transported member realization can be
reused, while \ruleref{loc-inter} combines the two realizations of the first
location:
\[
\begin{aligned}
  &\action{\ruleuse{c-pair-first-inter}}
    {\ruleuse{ref-loc}(\ruleuse{loc-inter}(R_L,R_R))}
  \\[2pt]
  &\qquad=\ruleuse{ref-loc}\bigl(
    \ruleuse{loc-pair}(B_x,\ruleuse{loc-inter}(R_S,R_T),R_\tau)\bigr).
\end{aligned}
\]
Thus the result exposes the one stored first component at
\(\interty{S}{T}\) without changing the physical pair or its member.
...
instantiation. Rules \ruleref{c-pair-first-inter},
\ruleref{c-pair-inter}, \ruleref{c-pair-type-inter}, and
\ruleref{c-pair-type-union-inter} each have coercion size one and perform no
recursive coercion application: they use store-binding uniqueness and rebuild
realization evidence directly. These are all recursive coercion applications.
...
two-member regression intersects aligned record spines, merges their differing
tails, normalizes the older same-label member with the existing term-member
rule, and then selects that member through the outer extension. A
...
The aligned first-component rule makes the same normalization compositional
over a record spine. Let
\[
  \mathsf{Tail}(X)=\pairty{x}{\top}{a}{X},
  \qquad
  \mathsf{Outer}(X)=
    \pairty{x}{\mathsf{Tail}(X)}{b}{\top},
  \qquad a\ne b.
\]
The two outer views have the same \(b\)-member but different record tails, so
\ruleref{s-pair-first-inter} gives
\[
  \interty{\mathsf{Outer}(L)}{\mathsf{Outer}(R)}
  <:
  \pairty{x}{
    \interty{\mathsf{Tail}(L)}{\mathsf{Tail}(R)}}{b}{\top}.
\]
Rule \ruleref{s-pair-inter} normalizes the first-component intersection to
\(\mathsf{Tail}(\interty{L}{R})\), and pair covariance transports the
unchanged \(b\)-member across that step. The mechanized program allocates one
physical \(a\)-then-\(b\) pair spine, gives one path-only alias the resulting
precise outer type, and selects \(a\). Path lookup skips \(b\) and exposes the
older member at \(\interty{L}{R}\); its two projections again type the
self-application. Conversely, pair covariance and meet introduction derive
the reverse direction of \ruleref{s-pair-first-inter}, so the aligned shape is
preserved by intersection up to mutual subtyping.
...
first-component merge for aligned pair spines, a term-member merge for an
identical first-component type and label, and both a shared-lower and a general
type-member merge. The first rule gives one restricted distributivity law for
records with an unchanged outer member; the general interval rule joins lower
bounds and meets upper bounds. Using the latter's target in term typing
separately requires the resulting interval to be consistent. These
connectives do not generally distribute through other type constructors or
participate directly in precise path synthesis, and the proper-type union is
not a sum type. The resulting runtime makes allocation order available as a
...
for each connective, the aligned first-component merge with an unchanged
member, the term-member merge for an identical first-component type and label,
the shared-lower type-member merge, and the general rule that joins lower
bounds while meeting upper bounds. The general rule does not discharge target
interval consistency; term subsumption still requires its separate
well-formedness proof. The calculus has no path-typing projection through
either connective, merge for unequal labels or simultaneous changes to both
pair components, other distributivity, dedicated term introduction or
