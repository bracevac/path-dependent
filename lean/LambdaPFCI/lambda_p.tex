...
Proper types also include binary intersections with the three standard meet
rules and restricted rules that merge pair views with an identical
first-component type and label. Proper term-member signatures are combined by
intersection; interval signatures with a shared lower bound are combined by
intersecting their upper bounds.
An intersection records two semantic realizations of one location; it adds no
value form, cast, or machine transition.
...
transitivity rule, binary meets, and bound-consistency premises for type
intervals.
...
\item a small calculus combining arbitrary stable paths, singleton types,
  binary intersections, abstract type selections, dependent functions, and
  dependent pairs with term and type members, including controlled merging of
  same-label term-member views and shared-lower type-member views;
...
The binary proper type \(\interty{S}{T}\) is controlled. Its three meet rules
introduce a meet from two views of the same source and eliminate either
component. Two additional rules merge pair views only when they have the same
...
\]
For interval type-member signatures, the rule additionally requires an
identical lower bound and intersects only the upper bounds:
\[
  \interty{\pairty{x}{S}{A}{L..U}}{\pairty{x}{S}{A}{L..V}}
  <:\pairty{x}{S}{A}{L..\interty{U}{V}}.
\]
This shared-lower restriction is material. Combining unrelated intervals
symmetrically while retaining both lower-bound guarantees would require a
lower bound above both source lowers, conventionally their union; unions are
not part of \lambdap{}.
There is no term-level intersection introduction: existing subsumption is the
only way to assign an intersection type to a term. In particular, path
synthesis is not extended with rules that project through intersections, and
the calculus has no general member merging, unrestricted distributivity
beyond these two pair rules, or intersection-specific record operation.
...
structural restrictions, and only when their lower bound agrees. Thus neither
rule merges differing labels or differing first-component types, and interval
members with distinct lower bounds remain separate. None of these rules
changes the precise type synthesized by
...
Intersections are absent from values, referents, continuations, and machine
states except as types decorating the metatheory. They add no path-resolution
or transition rule, and execution neither constructs intersection packages
nor performs casts. Thus the runtime syntax and operational semantics below
are unchanged by the intersection extension.
...
rules, and all eight signature well-formedness rules.}
...
subtyping.}
\Description{All sixteen signature-subtyping rules.}
\label{fig:subtyping}
\end{figure}
...
coercion composition. In particular, intersection conversion is
componentwise and neither reorders nor distributes its components.
...
\begin{figure}[H]
\setlength{\afterruleskip}{1.7em plus 0.2em minus 0.1em}
\noindent
\begin{minipage}[t]{0.48\linewidth}
...
\Description{All six path-equality rules, all eight signature-conversion rules,
both referent rules, and all six location-realization rules.}
...
\begin{figure}[H]
\ContinuedFloat
\setlength{\afterruleskip}{0.5em plus 0.05em minus 0.05em}
\noindent
\begin{minipage}[t]{0.48\linewidth}
...
rule, all seventeen semantic-coercion rules, and all five open-codomain rules.}
\end{figure}
...
value beyond those already imposed by its premises. Rule \ruleref{loc-fun} records the stored abstraction and its
...
Rules \ruleref{c-pair-inter} and \ruleref{c-pair-type-inter} are likewise
nullary. Their actions invert the two pair realizations stored by
...
term-member location. The latter identifies one stored type witness, reuses a
coercion from the shared lower bound to that witness, and intersects the two
coercions from the witness to the advertised upper bounds.
...
component realizations independently and rebuilds \ruleref{loc-inter}. In the pair
...
\infax[\ruleuse{s-inter-left}]
  {\compilepi{\mathcal R}{\pi}=\ruleuse{c-inter-left}}
\end{minipage}\hfill
\begin{minipage}[t]{0.26\linewidth}
\typicallabel{s-inter-right}
\infax[\ruleuse{s-inter-right}]
  {\compilepi{\mathcal R}{\pi}=\ruleuse{c-inter-right}}
\end{minipage}
...
\endgroup
\caption{Interpretation of all sixteen subtyping rules as semantic coercions.}
\Description{The sixteen equations defining subtyping interpretation. The
...
the common first-component location. With distinct source lowers, neither
lower coercion would in general establish the target lower bound; the
calculus deliberately provides no rule for that case.
...
The transitivity, intersection-introduction, and selection cases recurse on
coercions appearing as premises of the current coercion derivation. The
...
\ruleref{c-inter}, \ruleref{c-sel-lo}, and \ruleref{c-sel-hi} retain the
referent and recurse on a structurally smaller coercion, decreasing the
secondary component. The intersection projections do not recurse. In
...
instantiation. Rules \ruleref{c-pair-inter} and
\ruleref{c-pair-type-inter} each have coercion size one and perform no
...
and the two projections of its intersected upper bound.
...
\citep{amin2017definitional}. \lambdap{} removes recursive objects and unions
and studies paths built from immutable dependent pairs. It retains controlled
binary intersections with the three meet rules, a term-member merge for an
identical first-component type and label, and a corresponding type-member
merge restricted to a shared lower bound. Unlike richer DOT intersections,
they do not generally merge members, distribute through other type
constructors, or participate directly in precise path synthesis. The
...
would become relevant when extending \lambdap{} with sums and pattern
...
singleton types, controlled binary intersections, abstract type members with
interval signatures, explicit subtyping transitivity, and covariance of both
components of dependent pairs.
...
The intersection extension is intentionally limited to meet introduction and
elimination, the term-member merge for an identical first-component type and
label, and the analogous type-member merge when the lower interval bound is
shared. It has no path-typing projection, merging for intervals with distinct
lower bounds, merging for unequal labels or unequal first-component types,
other distributivity, term introduction form, or operational rule; extending
any of those dimensions would be a separate calculus change.
