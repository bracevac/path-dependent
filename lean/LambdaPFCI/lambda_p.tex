...
rules and a restricted rule that merges term-member pair views with an identical
first-component type and label.
...
\item a small calculus combining arbitrary stable paths, singleton types,
  ...
  term-member views with an identical first-component type and label;
...
component. One additional rule merges two pair views only when they have the
same first-component type, the same label, and proper term-member signatures:
\[
...
beyond this one pair rule, or intersection-specific record operation.
...
of two pair views whose first-component type and label agree. It does not
apply to interval members, differing labels, or differing first-component
types. None of these rules changes the precise type synthesized by
...
subtyping.}
\Description{All fifteen signature-subtyping rules.}
\label{fig:subtyping}
\end{figure}
...
rule, all sixteen semantic-coercion rules, and all five open-codomain rules.}
\end{figure}
...
Rule \ruleref{c-pair-inter} is likewise nullary. Its action inverts the two
pair realizations stored by \ruleref{loc-inter}, uses functionality of store
lookup to identify their one stored pair, and combines the two realizations
of that pair's term-member location.
...
\endgroup
\caption{Interpretation of all fifteen subtyping rules as semantic coercions.}
\Description{The fifteen equations defining subtyping interpretation. The
...
The restricted pair merge also acts without a run-time operation. Suppose the
two components of an intersection realization invert to
\[
\begin{array}{rcl}
...
instantiation. Rule \ruleref{c-pair-inter} has coercion size one and performs
no recursive coercion application: it uses store-binding uniqueness and
rebuilds realization evidence directly. These are all recursive coercion
applications.
...
restricted merge of those views into one selectable member intersection.
...
binary intersections with the three meet rules and one term-member merge for
an identical first-component type and label. Unlike richer DOT intersections,
...
elimination plus the term-member merge for an identical first-component type
and label. It has no path-typing projection, general member merging (in
particular for intervals, unequal labels, or unequal first-component types),
distributivity beyond this one pair rule, term introduction form, or
operational rule; extending any of those dimensions would be a separate
calculus change.
