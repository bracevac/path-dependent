...
inductive Tm.Cont.Evidence :
    {n : Nat} -> Store n -> LambdaPFCI.Ty n -> Tm.Cont n ->
      LambdaPFCI.Ty n -> Type 1 where
| hole {n : Nat} {sigma : Store n} {T : LambdaPFCI.Ty n} :
    Tm.Cont.Evidence sigma T [] T
| cons {n : Nat} {sigma : Store n} {S U V T : LambdaPFCI.Ty n}
...
inductive State.Evidence :
    {n : Nat} -> State n -> LambdaPFCI.Ty n -> Type 1 where
| ok {n : Nat} {sigma : Store n} {cont : Tm.Cont n}
    {term : Tm n} {S T : LambdaPFCI.Ty n} :
