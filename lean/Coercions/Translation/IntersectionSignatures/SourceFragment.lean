...
/-!
# The explicit stable intersection-signature source fragment
...
namespace DotToFCsub.IntersectionSignatures.SourceFragment
...
def layout {sourceScope : DotFC.Sig} {sourceContext : Ctx sourceScope}
    ...
    ContextLayout.Context (sourceContext.snoc type)
...
structure StableSelection {sourceScope : DotFC.Sig}
  ...
  allocated : ContextLayout.MemberAllocated layout path label
...
def lookup {sourceScope : DotFC.Sig} {sourceContext : Ctx sourceScope}
    ...
    ContextLayout.MemberLookup layout ⟨path, label⟩ :=
...
/-- Types for which intersection-signature layout translation is total.  The two function cases
...
inductive StableType : {sourceScope : DotFC.Sig} →
  ...
  | allPlain {sourceScope : DotFC.Sig}
      ...
      (shape : ContextLayout.PlainShape domain)
      (codomainStable :
        StableType (ContextLayout.Context.plain layout shape) codomain) :
...
inductive StableContext : {sourceScope : DotFC.Sig} →
  ...
  | nil : StableContext ContextLayout.Context.nil
  | plain {sourceScope : DotFC.Sig}
      ...
      (shape : ContextLayout.PlainShape type) :
      StableContext (ContextLayout.Context.plain outer shape)
  | signature {sourceScope : DotFC.Sig}
      {sourceContext : Ctx sourceScope} {targetScope : FCsub.Sig}
      {outer : ContextLayout.Context sourceContext targetScope}
      {type : Ty sourceScope}
...
end DotToFCsub.IntersectionSignatures.SourceFragment
