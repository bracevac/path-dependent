...
/-!
# Scoped closure of the intersection-signature bridge

This file closes the proof-producing boundary used by the variable-path
intersection-signature slice. Closed bounds are witnessed explicitly, successful signature
...
namespace DotToFCsub.IntersectionSignatures.Closure
...
/-- A supported signature has a total scope-polymorphic intersection-signature encoding. -/
...
noncomputable def newestMemberAllocated
    ...
    {targetScope : FCsub.Sig} (outer : ContextLayout.Context sourceContext targetScope)
    ...
    MemberAllocated (ContextLayout.Context.signature signature outer collectible closed)
      .here label := by
...
inductive PathWeakening :
  ...
  | plain {sourceScope sourceContext targetScope}
      ...
      PathWeakening outer path (ContextLayout.Context.plain middle shape)
        (.there middlePath)
  | signature {sourceScope sourceContext targetScope}
      ...
      PathWeakening outer path
        (ContextLayout.Context.signature signature middle collectible closed)
        (.there middlePath)
...
def weakenAlong {sourceScope : DotFC.Sig} {sourceContext : Ctx sourceScope}
    ...
    (allocated : ContextLayout.MemberAllocated outer path label)
    {laterSourceScope : DotFC.Sig} {laterContext : Ctx laterSourceScope}
    {laterTargetScope : FCsub.Sig}
    {later : ContextLayout.Context laterContext laterTargetScope}
    {laterPath : DotFC.BVar laterSourceScope .term}
    (weakening : PathWeakening outer path later laterPath) :
    ContextLayout.MemberAllocated later laterPath label :=
...
end DotToFCsub.IntersectionSignatures.Closure
