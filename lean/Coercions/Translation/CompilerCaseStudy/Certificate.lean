...
/-!
# Proof-directed compiler case-study certificate boundary
...
below is an existing recursive-object or path-alias certificate.  In particular,
this module does not define a second source typing relation and does not claim a
total compiler for `DotFCRP` terms.
-/
...
def leftKey (program : Surface.Program) :
    PathAliases.MemberKey program.pathScope :=
...
recursive-object/path-alias bridge.
...
structure Certificate (program : Surface.Program) : Type where
  ...
  encoding : RecursiveObjects.Encoding (target := []) program.definitions
  layout : PathAliases.PathLayout program.aliases []
  recursiveLayout : PathAliases.RecursiveLayoutRealization layout encoding
  leftImage : PathAliases.MemberImage layout program.leftKey
  rightImage : PathAliases.MemberImage layout program.rightKey
  leftMember : PathAliases.RecursiveMemberAt encoding leftImage
  rightMember : PathAliases.RecursiveMemberAt encoding rightImage
  memberEquality : PathAliases.MemberPathEq leftImage rightImage
...
/-- Reuse the exact recursive-object source-to-target preservation theorem for
the closed recursive object.  This is the complete source claim made by
CompilerCaseStudy. -/
noncomputable def sourcePreservation {program : Surface.Program}
    (certificate : Certificate program) :
    RecursiveObjects.RecursivePreservation certificate.encoding :=
  RecursiveObjects.preserveRecursiveObject certificate.sourceValid certificate.encoding

/-- The supplied complete path layout realizes the aliased recursive object
using the existing path-alias theorem. -/
noncomputable def aliasedObjectRealization {program : Surface.Program}
    (certificate : Certificate program) :
    PathAliases.AliasedRecursiveObjectRealization certificate.layout
      certificate.encoding FCsub.Ctx.nil :=
  PathAliases.realizeAliasedRecursiveObject certificate.recursiveLayout FCsub.Ctx.nil

/-- The selected source singleton equality is realized at the same exact
recursively encoded member on both paths, with both directed transports checked
by FCsub. -/
noncomputable def singletonRealization {program : Surface.Program}
    (certificate : Certificate program) :
    PathAliases.SingletonMemberRealization certificate.leftMember
      certificate.rightMember certificate.memberEquality FCsub.Ctx.nil :=
  PathAliases.realizeSingletonMember certificate.leftMember certificate.rightMember
