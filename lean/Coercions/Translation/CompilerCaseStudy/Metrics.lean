import Coercions.FCsub.ClosedArtifact
import Coercions.DOT.Recursive
import Coercions.DOT.TraceablePaths
...
The module has no dependency on the compiler case study.  A compiler supplies
its own source-node count and generation counts to `Metrics.ofArtifact`; the
target and erased counts and checker result are then obtained independently
from the proof-free closed FCsub artifact.
-/

namespace DotToFCsub.CompilerCaseStudy
...
size so the recursive-object/path-alias formulas remain visible in reports. -/
...
/-- The recursive-object translation allocates one public name and two directed
constraints per source member. -/
def recursiveObject (members : Nat) : GenerationCounts :=
  { generatedNames := members
    generatedConstraints := 2 * members
    aliasPairs := 0 }

/-- The path-alias translation allocates one fresh name/equality pair per
syntactic path-member key.  The equality is recorded as an alias pair, not as
a directed recursive-object constraint. -/
def pathAliases (aliases : Nat) : GenerationCounts :=
  { generatedNames := aliases
    generatedConstraints := 0
    aliasPairs := aliases }

/-- Combined recursive-member and traceable-path-alias accounting. -/
def recursiveObjectAndPathAliases (members aliases : Nat) : GenerationCounts :=
  (recursiveObject members).add (pathAliases aliases)

@[simp]
theorem recursiveObject_generatedNames (members : Nat) :
    (recursiveObject members).generatedNames = members := rfl

@[simp]
theorem recursiveObject_generatedConstraints (members : Nat) :
    (recursiveObject members).generatedConstraints = 2 * members := rfl

@[simp]
theorem pathAliases_generatedNames (aliases : Nat) :
    (pathAliases aliases).generatedNames = aliases := rfl

@[simp]
theorem pathAliases_aliasPairs (aliases : Nat) :
    (pathAliases aliases).aliasPairs = aliases := rfl

end GenerationCounts

/-- Number of projections in the exact recursive-object block: one erased object
payload projection followed by one projection per public member. -/
def recursiveObjectSlots (members : Nat) : Nat := members + 1

/-- Number of heterogeneous target binders introduced by path-alias nested
`newtype`: one type name and one equality assumption per alias. -/
def pathAliasScopeBinders (aliases : Nat) : Nat := 2 * aliases
...
/-- Attach the generic recursive-object/path-alias resource formulas to a
closed artifact. -/
def forRecursiveObjectAndPathAliases (sourceNodes members aliases : Nat)
    (artifact : FCsub.ClosedArtifact) : Metrics :=
  ofArtifact sourceNodes
    (GenerationCounts.recursiveObjectAndPathAliases members aliases) artifact
...
end DotToFCsub.CompilerCaseStudy
