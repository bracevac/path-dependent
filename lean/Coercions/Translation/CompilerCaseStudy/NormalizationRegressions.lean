import Coercions.FCsub.Normalization
import Coercions.Translation.PathAliases.Examples

/-!
# Compiler case-study normalization regressions

These checks specialize the standalone FCsub normalizer to the concrete
singleton-member equality emitted by the nested path-alias example.  The source
...
namespace DotToFCsub.CompilerCaseStudy.NormalizationRegressions

open DotToFCsub.PathAliases
open DotToFCsub.PathAliases.Examples

/-- The concrete path-alias certificate contains six equality nodes:
...
/-- Re-normalizing the concrete path-alias certificate is a fixed point. -/
...
certificate at exactly the endpoints used by the original path-alias regression. -/
...
end DotToFCsub.CompilerCaseStudy.NormalizationRegressions
