...
The path-alias translation deliberately allocates by the syntactic key
`(path, label)`.  Successful
...
an explicit coercion, developed separately in `CoResolvedEquality`; it never
changes this allocation table.

This module also connects a finite path layout to the exact recursive-member
interface constructed in `RecursiveObjects`.  Each source slot retains its
transparent trace and identifies its ambient target anchor with one recursive
projection.
-/
...
/-! ## Specialization to the exact recursive-member interface -/

/-- Ambient identity of one exact recursive member. -/
def recursiveAnchor {target : FCsub.Sig}
  ...
  .recProj encoding.block (DotToFCsub.RecursiveObjects.memberIndex index)

/-- The corresponding instantiated exact recursive witness. -/
...
/-- One allocated path member is realized by a public member of a
recursive interface.  The source trace is retained by `image.trace`; the
fields below align its label and target anchor with the exact recursive slot. -/
...
/-- Canonical recursive unfolding equality, renamed below all generated aliases. -/
def equality {source : DotFC.Sig} {store : AliasStore source}
  ...
  (FCsub.EqCo.unfoldRec encoding.block
    (DotToFCsub.RecursiveObjects.memberIndex realization.memberIndex)).rename
...
noncomputable def equality_hasType {source : DotFC.Sig}
  ...
  have typing := FCsub.EqCo.HasType.unfoldRec
    ...
    (index := DotToFCsub.RecursiveObjects.memberIndex realization.memberIndex) guarded
  ...
  have unfoldEqual :
      (encoding.block.rename (AliasScope.weaken layout.count)).unfoldAt
          (DotToFCsub.RecursiveObjects.memberIndex realization.memberIndex) =
        (recursiveExact encoding realization.memberIndex).rename
          (AliasScope.weaken layout.count) := by
    rw [← FCsub.RecBodies.unfoldAt_rename]
    simpa [recursiveExact, DotToFCsub.RecursiveObjects.Encoding.block,
      DotToFCsub.RecursiveObjects.Encoding.witnesses] using
        congrArg
...
/-- Exact lower transport: witness to recursive anchor, then anchor to alias. -/
...
/-- Exact upper transport: alias to recursive anchor, then anchor to witness. -/
