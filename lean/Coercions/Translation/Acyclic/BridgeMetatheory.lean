...
import Coercions.FCsub.Structural
...
stable-root bridge: selections use direct handles, function subtyping
preserves the plain/member interface split, and a member application uses the
canonical slot belonging to its argument path.
-/
