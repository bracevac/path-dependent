import Coercions.Translation.StableRoots.Translation
...
namespace DotToFCsub.StableRoots.Opening
...
inductive ContextOpening :
  ...
  | member {source : DotFC.Sig} {context : DotFC.Source.Ctx source}
      ...
      (argument : DotFC.BVar source .term)
      (argumentRoot : DotToFCsub.StableRoots.StableRoot context argument label)
...
private def previousRoot {source : DotFC.Sig}
    ...
    {path : DotFC.BVar source .term} {label : DotFC.Source.Name}
    (root : DotToFCsub.StableRoots.StableRoot (context.snoc bound)
      (.there path) label) :
    DotToFCsub.StableRoots.StableRoot context path label := by
...
private def weakenRoot {source : DotFC.Sig}
    ...
    (root : DotToFCsub.StableRoots.StableRoot context path label) :
    DotToFCsub.StableRoots.StableRoot (context.snoc bound) (.there path) label :=
...
private theorem memberHereLabel {source : DotFC.Sig}
    ...
    (root : DotToFCsub.StableRoots.StableRoot
...
noncomputable def transportRoot {source target : DotFC.Sig}
    ...
    (opening : ContextOpening before after rho substitution)
    {path : DotFC.BVar source .term} {label : DotFC.Source.Name}
    (root : DotToFCsub.StableRoots.StableRoot before path label) :
    DotToFCsub.StableRoots.StableRoot after (rho.var path) label := by
...
theorem translateTy_substitute {source target : DotFC.Sig}
    ...
    (stable : DotToFCsub.StableRoots.StableWf valid formation) :
  ...
  | @DotToFCsub.StableRoots.StableWf.all _ _ _ domain codomain _ _
...
theorem openPlain_substitute {source : DotFC.Sig}
    ...
    (codomainStable : DotToFCsub.StableRoots.StableWf
...
end DotToFCsub.StableRoots.Opening
