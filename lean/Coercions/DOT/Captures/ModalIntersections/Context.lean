...
/-!
# Contexts for cumulative captured DOT

The cumulative context follows the heterogeneous source scope exactly.
Runtime binders carry combined source types; lexical type and capture binders
carry true intervals. Stable object roots need no separate binding form: a
term binding whose type exposes an object retains its complete interface.
-/

namespace DOTCapture.ModalIntersections

/-- The payload associated with one source binder. -/
...
def weaken {scope : Sig} {kind newest : BinderKind}
  ...
  binding.rename DOTCapture.BinderOnly.Rename.succ

@[simp]
theorem rename_id {scope : Sig} {kind : BinderKind}
    (binding : Binding scope kind) :
    binding.rename DOTCapture.BinderOnly.Rename.id = binding := by
...
/-- A source context aligned with its heterogeneous scope. -/
...
/-- Kind-correct lookup, weakened into the complete ambient scope. -/
