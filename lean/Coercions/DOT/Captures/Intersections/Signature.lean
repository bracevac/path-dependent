...
This module is the source-independent collection layer for type and capture
member intersections.  A signature has at most one entry for each natural-
number label.  Every interval occurrence contributing to that label is kept
in the entry; merge never invents a combined endpoint.

The bound-expression family is abstract.  Instantiating `Expr .type` and
`Expr .capture` with a captured-DOT source language is a later layer.
-/
