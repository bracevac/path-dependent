...
inductive Proposition : Relation → Sig → Type where
  ...
  /-- Every capability in `capture` belongs to the ground classifier kind.
  Classifier kinds remain closed annotations rather than static expressions. -/
  | captureHasKind {scope : Sig} (capture : Capture scope)
      (kind : Classifier.Kind) : Proposition .captureHasKind scope
...
def Proposition.rename {relation : Relation} {source target : Sig}
  ...
  | .captureHasKind capture kind =>
      .captureHasKind (capture.rename rho) kind
