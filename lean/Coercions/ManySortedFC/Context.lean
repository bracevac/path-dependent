...
/-- Enter the proof-only assumptions generated by a primitive modal lock. -/
def extendModal {scope : Sig} {separationCount : Nat}
    {modes : List CaptureMode} (context : Ctx scope)
    (requirements : ModalContext separationCount modes scope) :
    Ctx (ModalScope scope separationCount modes) :=
  context.extendTheory requirements.toTheory
