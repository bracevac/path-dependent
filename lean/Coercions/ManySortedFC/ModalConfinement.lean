...
inclusion, ground capture-kinding, and disjointness certificates can be
lowered through the modal evidence block and therefore cannot depend on any
assumption introduced by the lock.
-/

namespace ManySortedFC

/-- Primitive modal contexts contain only mode and separation assumptions,
so they cannot introduce a capture-kinding hypothesis. -/
...
/-- Lower ground capture-kinding evidence through a modal proof block. -/
...
def lowerModalCaptureHasKind_rename {scope : Sig}
  ...
  | .captureHasKindWiden membership sourceKind targetKind => by
      simp only [lowerModalCaptureHasKind, Evidence.rename,
        lowerModalCaptureHasKind_rename membership]
termination_by nodeCount evidence
decreasing_by
...
/-- Checked ground capture-kinding derivations under a lock lower to the
...
noncomputable def lowerModalCaptureHasKind {scope : Sig}
  ...
  | .captureHasKindEmpty kind => by
      simpa [Evidence.lowerModalCaptureHasKind, Proposition.substitute,
        ...
        (Evidence.Proves.captureHasKindEmpty (context := context) kind)
  | .captureHasKindUnion left right => by
      simpa [Evidence.lowerModalCaptureHasKind, Proposition.substitute,
        Capture.substitute] using
  ...
  | .captureHasKindWiden membership kindSubtyping => by
      simpa [Evidence.lowerModalCaptureHasKind, Proposition.substitute,
        Capture.substitute] using
        Evidence.Proves.captureHasKindWiden
          (lowerModalCaptureHasKind membership) kindSubtyping
