...
The body of a recursive type is restricted to declaration-shaped types: $\top$, type members, fields, intersections of those, and recursive types with declaration-shaped bodies. The predicate $\Decl$ states this, and it is decidable. Well-formedness $\Wf$ is structural except that $\dmu{x}{T}$ requires a declaration-shaped body. Intersections are unrestricted. Bounds inside a declaration are arbitrary well-formed types, and $\Wf(\dtyp{A}{S}{T})$ does not ask for $S \sub T$. The labels of a definition list must be distinct. Nothing else about DOT is restricted.

The restriction excludes a recursive type whose body is a type selection, a function type, $\bot$, or an intersection with such an operand, as in $\dmu{x}{x.A \wedge \dfld{a}{T}}$. The refinement of an abstract type without a self binder, $x.A \wedge \dfld{a}{T}$, is admitted. It is the form by which DOT refines a class type with a declaration. The reason for the restriction is on the target side. An object type is a telescope of propositions over a self block, and a type that is not a declaration enters a telescope as one proposition, that the object itself is included in that type. Such a proposition is a closed type and cannot mention the self, so a body that is not a declaration has no telescope over its own self. Definition typing only ever produces declarations, so every literal has a declaration-shaped body. What the restriction rejects is a recursive type written over another shape, and the folding of a refinement back under a self binder by \rn{Rec-I}. The first version of the calculus restricted intersections to declaration-shaped operands as well, and Section~\ref{sec:forced} says what lifting that took.

Figure~\ref{fig:source-typing} gives subtyping, typing, and definition typing. The three judgments are mutually inductive, because \rn{Sel-<:} and \rn{<:-Sel} have typing premises and a variable typing may go through subsumption. The rules that open or close a recursive type carry $\Decl$ premises, which is where the fragment is enforced. Well-formedness appears only where a type is introduced out of thin air, at the domain of a lambda and at the result of a let. In the mechanization the three judgments are inductive families in $\mathsf{Type}$, because the translation is a function on derivations.
