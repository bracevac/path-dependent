...
\paragraph{Axioms.}
The theorems of Sections~\ref{sec:canonical} and~\ref{sec:translation} depend on propositional extensionality and quotient soundness only. The two case splits that could have needed choice, whether an atom is a bare variable in progress and whether a state is about to move a cast frame in the backward erasure simulation, are decided by a derived equality on atoms and by an executable test for the cast-frame predicate. The tree contains no unproven assumption, no admitted lemma, and no unbounded recursion. The pigeonhole argument behind resolution is proven from scratch, because the core library has no lemma for it and the nearby list lemmas depend on choice.
