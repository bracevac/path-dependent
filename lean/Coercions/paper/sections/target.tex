...
\text{Morphisms} & m & ::= & \ev{nil} \mid \ev{le}\,m\,q\,H\,q \mid \ev{eq}\,m\,j\,b \mid \ev{has}\,m\,j \mid \ev{bnd}\,m\,e \\[2pt]
...
\]
\color{black}
\caption{Syntax of \FCdot. In $\ev{pi}\,e\,e'$ the second component lives under one more binder. In $\obj{\tel}$, and in the telescopes carried by evidence and atoms, it is the telescope arguments that live under one more binder.}
\label{fig:target-syntax}
\end{figure*}

Figure~\ref{fig:target-syntax} gives the syntax. As in the source, everything is intrinsically scoped, and the two calculi share signatures, variables, renamings, and the label type. The source is built on the target's binder infrastructure, so the translation is the identity on signatures. This is a convenience of the mechanization, and nothing in the evidence design depends on it.

\subsection{Types}

A type is $\bot$, a block name $\sel{x}{\ell}$, a dependent function type $\pit{S}{T}$ whose codomain may mention the parameter's block, or an object type $\obj{\tel}$. A telescope is a list of propositions over a self binder, oldest first. Propositions do not bind, so the telescope is a list rather than a dependent context, and $\tat{\tel}{i}{P}$ says that its $i$-th proposition is $P$. The empty object type asserts nothing about its inhabitants and every type is included in it, so $\top$ is defined as $\obj{\tnil}$. The fourth kind of proposition, a self-bound $\pbnd{T}$, says that the object itself is included in $T$. By convention the type of a bound is a closed type weakened under the self binder, so a bound never mentions the self. The rules that prove a bound only ever prove such a bound, and the rule that uses one only reads such a bound. Bounds are what a type that is not a declaration becomes inside a telescope, and Section~\ref{sec:translation} uses them for intersections with a selection, a function type, or $\bot$ as an operand. Types never contain terms: the only way a type refers to a binder is through the names of its block. Every label has a name in every block, and $\sel{x}{a}$ for a term label $a$ is the type of the field $a$ of $x$.
...
\and
\inferrule{\morj{\Gamma}{m}{\tel}{\tel'} \\ \lej{\Gamma}{e}{\obj{\tel}}{T}}
  {\morj{\Gamma}{\ev{bnd}\,m\,e}{\tel}{\tcons{\tel'}{\pbnd{\weak{T}}}}}
\end{mathpar}
\color{black}
...
The elimination rule $\ev{member}$ is the one rule that connects the type of an atom to the block of its root, the variable under its wrappers, defined in Section~\ref{sec:atoms}. From $a : S$ and evidence $e$ for $S \le \obj{\tel}$, the $i$-th proposition of $\tel$ holds at $\rt{a}$, as an inclusion, an equality, or a presence, according to its kind. The rule $\ev{def}$ reads the definition of a block name off a transparent binder, and $\Gamma(x) \ni \ell \mathrel{:=} W$ says that $x$ is transparent in $\Gamma$ with witness $W$ at $\ell$. Contexts have two kinds of binder: an opaque one, $x : T$, whose block is abstract, and a transparent one, $x : T \mathrel{:=} W ; \vec\ell$, whose block is defined by the witnesses $W$ and which is known to have the fields $\vec\ell$. A witness list assigns $\top$ to every label it does not mention. Transparent binders arise in two places: inside an object literal, where the fields are typed under the literal's own self binder, and in store typing, where every allocated binder is transparent.
...
A self-bound has one elimination, one introduction, and one entry in morphisms. The cast $\ev{bound}\,\tel\,i$ takes the object type $\obj{\tel}$ through its $i$-th bound. The evidence $\ev{intoBnd}\,e$ turns $S \le T$ into $S \le \obj{[\,\pbnd{\weak{T}}\,]}$, the object type with that one bound. And a morphism proves a target bound by $\ev{bnd}\,m\,e$, where $e$ is closed evidence out of the source object type as a whole. This is the one entry of a morphism that reads the whole source rather than one of its propositions, and it still eliminates nothing through the self block, since $e$ is closed.
...
An atom is a variable under wrappers that erase to nothing: a cast, the unfolding of a self block at the root, the folding of the root back into a self block, and the pairing of two typings of one variable. The root of an atom is the variable underneath:
\[
\begin{array}{@{}rcl@{}}
\rt{x} & = & x \\
\rt{\ev{cast}\,a\,e} & = & \rt{a} \\
...
\mst{\sigma}{K}{a\,b} &\step& \mst{\sigma}{K}{\inst{t_0}{b}} & \text{if } \sigma(\rt{a}) = \lambda(z : S_0).\,t_0,\ a \text{ not a variable},\ \chn{\sigma}{a}{n}{(a', F)},\ F \in \{\form{id}, \form{eqv}\,\varphi\} \\
\mst{\sigma}{K}{a\,b} &\step& \mst{\sigma}{K}{\ev{cast}\,(\inst{t_0}{\ev{cast}\,b\,d})\,(\inst{c}{b})} & \text{if } \sigma(\rt{a}) = \lambda(z : S_0).\,t_0,\ a \text{ not a variable},\ \chn{\sigma}{a}{n}{(a', \form{pi}\,d\,c)} \\
...
Figure~\ref{fig:machine} gives the steps. Allocation is the rule that keeps stores inert: the value that reaches a let frame may carry casts, and the machine stores the literal underneath at its own type and rewrites the continuation so that the new variable is used under the composite of the stripped casts. Application through a bare variable is ordinary $\beta$. Application through a wrapped atom reads the domain and codomain evidence off the normal form of the atom's chain of casts. When that form is the identity or a conversion, the closure is applied directly. When it is a function coercion $\form{pi}\,d\,c$, the argument is cast by $d$ and the result by $c$ instantiated at the argument. The machine thus runs the normalizer of Section~\ref{sec:canonical}, and progress has to show that the normalizer succeeds on a closed atom of function type. Projection reads the field off the literal at the root and instantiates the field's self binder by the root.
