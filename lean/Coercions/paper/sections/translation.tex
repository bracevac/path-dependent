...
\trans{\dmu{x}{T}} & = & \obj{(\mathit{tel}_{\self}\,\src{T})} \\[4pt]
\mathit{tel}\,\src{\top} & = & \tnil \qquad\qquad \mathit{tel}\,\src{(S \wedge T)} \ =\ \mathit{tel}\,\src{S} \mathbin{+\!\!+} \mathit{tel}\,\src{T} \\
\mathit{tel}\,\dtyp{A}{S}{T} & = & [\,\ple{\weak{\trans{S}}}{\sel{\self}{A}},\ \ple{\sel{\self}{A}}{\weak{\trans{T}}}\,] \qquad\qquad \mathit{tel}\,\dfld{a}{T} \ =\ [\,\phas{a},\ \ple{\sel{\self}{a}}{\weak{\trans{T}}}\,] \\
\mathit{tel}\,\dmu{x}{T} & = & \mathit{tel}_{\self}\,\src{T} \quad \text{if } \Decl\,\src{T} \\
\mathit{tel}\,\src{B} & = & [\,\pbnd{\weak{\trans{B}}}\,] \quad \text{for every other } \src{B}
\end{array}
\]
\caption{Translation of types. $\mathit{tel}\,\src{T}$ is the telescope of a type over a fresh self, and $\mathit{tel}_{\self}\,\src{T}$ reads a body whose self is the innermost binder as propositions about that binder, so that the self of $\dmu{x}{T}$ becomes the self block of the object type. A type that is not an object shape enters a telescope as one self-bound.}
\label{fig:trans-types}
\end{figure*}

Figure~\ref{fig:trans-types} gives the translation of types. Call $\src{T}$ an object shape when $\trans{T} = \obj{\mathit{tel}\,\src{T}}$: that is $\src{\top}$, a declaration, an intersection, or a recursive type with a declaration-shaped body. Every other type enters a telescope as one self-bound, so an intersection is a concatenation of telescopes whatever its operands are, and $\trans{x.A \wedge \dfld{a}{T}}$ is the object type with the bound $\pbnd{\weak{\sel{x}{A}}}$ followed by the two propositions of the field. Every bound the translation produces is closed. A recursive type binds its own self, which is identified with the self block of the object type, so $\dmu{x}{T}$ and its opening at a variable $\src{x}$ translate to an object type and its unfolding at $x$. The two telescope readings agree on weakened types. A recursive type whose body is not declaration-shaped is not well formed, but it is a type, and its telescope is the single bound $\pbnd{\weak{\trans{\dmu{x}{T}}}}$, so that $\mathit{tel}$ is total and never produces a bound that mentions the self.

A declaration type $\src{T}$ of a literal determines the witnesses of the translated literal: the exact bound of each type member and the declared type of each field. Its precise type $\tgt{\mathit{litTy}(\src{T})}$ is the object type of the telescope generated from those witnesses and the field labels of $\src{T}$. Contexts translate binder by binder: an ordinary binder $\src{x : T}$ becomes an opaque binder at $\trans{T}$, and a literal's self binder $\src{x : \dmu{x}{T} \mathrel{:=} d}$ becomes a transparent binder at $\tgt{\mathit{litTy}(\src{T})}$ with the witnesses and field labels of $\src{T}$.
...
\trans{\rn{And}_1} & = & \begin{cases} \ev{obj}\,(\mathit{tel}(\src{S \wedge T}))\,(\mathit{idMor}_0\,(\mathit{tel}\,\src{S})) & \src{S} \text{ an object shape} \\ \ev{bound}\,(\mathit{tel}(\src{S \wedge T}))\,0 & \text{otherwise} \end{cases} \\
\trans{\rn{And}_2} & = & \begin{cases} \ev{obj}\,(\mathit{tel}(\src{S \wedge T}))\,(\mathit{idMor}_{|\mathit{tel}\,\src{S}|}\,(\mathit{tel}\,\src{T})) & \src{T} \text{ an object shape} \\ \ev{bound}\,(\mathit{tel}(\src{S \wedge T}))\,|\mathit{tel}\,\src{S}| & \text{otherwise} \end{cases} \\
\trans{\rn{And}\,d_1\,d_2} & = & \ev{pair}\,(\mathit{tel}\,\src{T})\,(\mathit{tel}\,\src{U})\,(\mathit{into}_{\src{T}}\,\trans{d_1})\,(\mathit{into}_{\src{U}}\,\trans{d_2}) \\
...
\trans{\rn{Var}}_a & = & x, \text{ or } \ev{cast}\,x\,\weak{(\mathit{litCo}\,\src{T})} \text{ when } \src{x : \dmu{x}{T} \mathrel{:=} d} \text{ in } \src{\Gamma} \\
...
\trans{\rn{And-I}\,h_1\,h_2}_a & = & \ev{both}\,(\mathit{tel}\,\src{T})\,(\mathit{tel}\,\src{U})\,(\mathit{into}_{\src{T}}\,\trans{h_1}_a)\,(\mathit{into}_{\src{U}}\,\trans{h_2}_a) \\
\trans{\rn{Sub}\,h\,d}_a & = & \ev{cast}\,\trans{h}_a\,\trans{d}
\end{array}
\]
\caption{Translation of subtyping derivations to inclusion evidence, $\trans{d}$, and of variable typings to atoms, $\trans{h}_a$. $\mathit{idMor}_k\,\tel$ is the morphism of identity templates whose holes name the propositions of $\tel$ at positions $k, k+1, \ldots$ of the source, a bound among them copied by $\ev{bnd}$ over $\ev{bound}$. $\mathit{into}_{\src{T}}$ is the identity when $\src{T}$ is an object shape, and otherwise $\ev{intoBnd}$ on evidence and $\ev{cast}\,a\,(\ev{intoBnd}\,(\ev{refl}\,\trans{T}))$ on an atom $a$.}
\label{fig:trans-evidence}
\end{figure*}

Figure~\ref{fig:trans-evidence} gives the translation of subtyping derivations and of variable typings, which are mutual because \rn{Sel-<:} and \rn{<:-Sel} have typing premises and a variable typing may go through subsumption. The structural rules go to the corresponding evidence. The projection of an intersection onto an operand that is an object shape is an object coercion whose morphism copies that half of the source by identity templates. The projection onto any other operand is $\ev{bound}$ at the operand's position, since that operand is one bound of the source telescope. Pairing is $\ev{pair}$, after each side is put into its telescope by $\mathit{into}$. The declaration rules \rn{Fld} and \rn{Typ} route each source proposition through the translated bound as a side of a template: the field rule inherits presence and adjusts the upper bound, and the type-member rule adjusts the lower bound contravariantly and the upper bound covariantly. The selection rules are $\ev{member}$ at the atom of the typing premise, on the proposition of the declaration that names the bound, and this is where the root of the atom matters: the atom of $\src{h : x : \dtyp{A}{S}{T}}$ has root $x$, so the eliminated proposition is about $\sel{x}{A}$. The \rn{All} rule needs no narrowing lemma. The codomain evidence is checked under a binder of the smaller domain, and the two codomains mention only that binder's block, which does not depend on its type.

A variable bound by an object literal is used through a cast from the literal's precise type to its declaration type, $\tgt{\mathit{litCo}\,\src{T}}$. That coercion is an object coercion whose morphism reads both bounds of each type member off the definition equality of the literal, one of them flipped, and inherits the presence of each field and reads its bound off the definition equality as well. The rules \rn{Rec-I} and \rn{Rec-E} unfold the atom at its root and refold it at the other telescope, \rn{And-I} is $\ev{both}$ on the two atoms put into their telescopes by a cast that keeps the root, and subsumption is a cast.
...
The typedness theorems hold for well-formed contexts. A context is well formed, $\src{\Gamma\ \mathit{wf}}$, when the declaration type $\src{T}$ of every literal binder $\src{x : \dmu{x}{T}}$ has literal shape, with exact type members and distinct labels. This is what \rn{\{\}-I} produces, and the empty context is well formed.

\begin{theorem}[Typedness]\label{thm:trans-typed}
Let $\src{\Gamma\ \mathit{wf}}$.
\begin{enumerate}
\item If $\src{d}$ derives $\subj{\Gamma}{S}{T}$ then
\[ \lej{\trans{\Gamma}}{\trans{d}}{\trans{S}}{\trans{T}} . \]
\item If $\src{h}$ derives $\dtyj{\Gamma}{x}{T}$ then
\[ \atomj{\trans{\Gamma}}{\trans{h}_a}{\trans{T}} \quad\text{and}\quad \rt{\trans{h}_a} = \src{x} . \]
\item If $\src{h}$ derives $\dtyj{\Gamma}{t}{T}$ then
\[ \tmj{\trans{\Gamma}}{\trans{h}}{\trans{T}} . \]
\end{enumerate}
\end{theorem}

The root conjunct of item 2 is what makes the selection rules type check: $\ev{member}$ instantiates the eliminated proposition at the root of the atom, and the source rule speaks about $\dsel{x}{A}$. The literal case of item 3 needs that the witnesses read off the declaration type return the translated declared type at every field, which follows from distinct labels, and that $\tgt{\mathit{litCo}}$ is typed, which is a computation on positions inside the literal's precise telescope.
...
If $\dtyj{\cdot}{t}{T}$ and the initial state $\src{\mst{\cdot}{\cdot}{t}}$ runs to a state $\src{\mathit{st}}$, then $\src{\mathit{st}}$ is final or steps.
\end{theorem}
...
Four things about the translation were found while proving it, not while designing it.
...
The first version of the proof needed a further side condition on \rn{\{\}-I}, the source side of the self-alias restriction of Section~\ref{sec:resolution}: the declared type of a field could not be a bare selection on the object's own self. The reason was on the target side. Resolution followed definitions by a well-founded measure, which required that no witness of a literal be a name of the same block, and a field declared at $\dsel{x}{A}$ inside its own literal makes the witness of $\sel{x}{a}$ the name $\sel{x}{A}$. The condition rejected literals such as $\src{\nu(x.\,\ddeftyp{T}{\mathit{Int}} \wedge \ddeftrm{v}{n})}$ typed at $\src{\dmu{x}{\dtyp{T}{\mathit{Int}}{\mathit{Int}} \wedge \dfld{v}{x.T}}}$, which DOT accepts. The fuel-and-pigeonhole resolution of Section~\ref{sec:resolution} accepts every literal, including the two-element cycle, and the restriction disappeared from the target's literal rule, from the source's \rn{\{\}-I}, and from the translation.
...
The first version of the translation restricted intersections to declaration-shaped operands, because the target had no self-bounds and an intersection with a selection as an operand had no telescope. Self-bounds lifted the restriction on the source side. What they cost on the target side is the routed entries of Section~\ref{sec:canonical}: the pairing of a bound cast with another coercion has entries that name propositions of an object type other than the source, which an $\form{obj}$ form cannot express. The clause of $\mathit{tel}$ for a recursive type consults $\Decl$ for the same reason that the fragment does. Without it, a recursive type whose body is not declaration-shaped, which \rn{And}$_1$ may still project, would produce a bound that mentions the self, and no rule of \FCdot proves or uses such a bound. With it every bound the translation produces is closed, which the typedness of the identity morphisms needs.
