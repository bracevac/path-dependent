...
`Coercions.Classifiers.FCdot.Checker` gives an executable kernel that returns the
...
Soundness lives in `Coercions.Classifiers.FCdot.Checker`; here it is paired with
