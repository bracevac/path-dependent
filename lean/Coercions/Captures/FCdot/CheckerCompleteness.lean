...
`Coercions.Captures.FCdot.Checker` gives an executable kernel that returns the
...
Soundness lives in `Coercions.Captures.FCdot.Checker`; here it is paired with
