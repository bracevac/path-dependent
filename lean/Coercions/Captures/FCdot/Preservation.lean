...
theorem Value.witnesses_rename {s1 s2 : Sig} :
  ...
  | .obj _ _ _, _ => by simp [Value.rename, Value.witnesses]
...
theorem Value.fieldLabels_rename {s1 s2 : Sig} :
  ...
  | .obj _ _ _, _ => by simp [Value.rename, Value.fieldLabels]
...
theorem Value.capWitnesses_rename {s1 s2 : Sig} :
  ...
  | .lam _ _, _ => by simp [Value.rename, Value.capWitnesses, CapWitnesses.rename]
  | .obj _ _ _, _ => by simp [Value.rename, Value.capWitnesses]
...
theorem Value.HasType.lam_inv {s : Sig} {Γ : Ctx s} {S₀ : Ty s} {t₀ : Tm (s,x)}
    ...
    ∃ T₀, T = (Π(S₀) T₀) ^ [] ∧ (Γ.cons (.opaque S₀)) ⊢ t₀ : T₀ := by
  cases h with
...
theorem Value.HasType.obj_inv {s : Sig} {Γ : Ctx s}
    ...
    (h : Γ ⊢ᵥ .obj W Wc F : T) :
    T = (μ (Telescope.ofLiteral W Wc F.labels)) ^ [] ∧
      Fields.HasType
        (Γ.cons (.transparent ((μ (Telescope.ofLiteral W Wc F.labels)) ^ []) W Wc F.labels)) F := by
  cases h with
  | obj hF => exact ⟨rfl, hF⟩

theorem Fields.HasType.get {s : Sig} {Γ : Ctx (s,x)} :
    ∀ (F : Fields (s,x)), Γ ⊢ᶠ F → ∀ (l : Label) (t : Tm (s,x)),
      F.get? l = some t → Γ ⊢ t : (.here ∙ l) ^ []
...
theorem Value.HasType.beta {s : Sig} {Γ : Ctx s} {S₀ S : Ty s} {t₀ : Tm (s,x)}
    {T : Ty (s,x)} {C : CaptureSet s} {b : Atom s}
    (hlam : Γ ⊢ᵥ .lam S₀ t₀ : (Π(S) T) ^ C) (hb : Γ ⊢ₐ b : S) :
    Γ ⊢ t₀.substAtom b : T⟦b.root⟧ := by
  obtain ⟨T₀, hTe, ht₀⟩ := Value.HasType.lam_inv hlam
  obtain ⟨-, rfl, rfl⟩ : C = [] ∧ S = S₀ ∧ T = T₀ := by
...
theorem Store.Typed.beta {s : Sig} {σ : Store s} {Γ : Ctx s} {x : BVar s .var}
    {S₀ S : Ty s} {t₀ : Tm (s,x)} {T : Ty (s,x)} {C : CaptureSet s} {b : Atom s}
    (hσ : ⊢ σ : Γ) (hx : σ.lookup x = .lam S₀ t₀) (hty : Γ.lookupTy x = (Π(S) T) ^ C)
...
theorem Tm.HasType.projField {s : Sig} {σ : Store s} {Γ : Ctx s} {y : BVar s .var}
    {W : Witnesses (s,x)} {Wc : CapWitnesses (s,x)} {F : Fields (s,x)} {ℓ : Label}
    {t : Tm (s,x)}
    (hσ : ⊢ σ : Γ) (hx : σ.lookup y = .obj W Wc F) (hg : F.get? ℓ = some t) :
    Γ ⊢ t.selfAt y : (y ∙ ℓ) ^ [] := by
  ...
  simpa [Tm.selfAt, Ty.rename, Shape.rename, CaptureSet.rename] using
    (Fields.HasType.get F hF ℓ t hg).rename hren
...
theorem preservation {s s' : Sig} {st : State s} {st' : State s'} {U : Ty s}
  ...
  case unboxRefl hx hcf hid =>
      ...
      | unbox ha hf =>
          ...
          obtain rfl : X = _ :=
            Shape.box.inj (hshape.symm.trans ((hF Γ hσ).boxRefl ha hcf hid))
          exact State.Typed.exists_rename_id ⟨Γ, _, hσ, .atom hb, hK⟩
  case unboxCast hx hcf =>
      cases ht with
      | unbox ha hf =>
          obtain ⟨X, hshape, hb⟩ := hσ.boxContent hx
          exact State.Typed.exists_rename_id
            ⟨Γ, _, hσ, .atom (hb.cast ((hF Γ hσ).boxed ha hcf hshape)), hK⟩
