...
| `Names` | the names of a capture set in two readings (`Ctx.namesCapsP`), the expansion of a root into every capture binder that is no root (`Ctx.expandNames`), kills and claims (`Ctx.killNames`, `Ctx.killFor`, `Ctx.claimsFor`, `Ctx.ownClosure`), the side conditions `Ctx.Accessible` (over effective liveness, `Ctx.EffLive`, decided by `Ctx.effLiveB`), `Ctx.ConsumeOk`, `Ctx.KillOk`, `Ctx.ArgSep` (ownership read both ways), `Ctx.AccessOnly`, consumed leaves (`Ctx.consumedLeaves`), the use mode (`CapAtom.useMode`: `consume` on a term binder or a capture name raises nothing), and the algebra of mode bounds |
| `NameMaps` | what a kill changes (a bit and nothing else), the evidence half of a map (`Ctx.KillMap`: flavours and bits), the term half (`Ctx.NamesMap`: names covered up to fresh binders, which a capture weakening appends), names up to ownership (`Ctx.NamesMapO`, with `Ctx.NamesMap.toO`), their transports, effective liveness under kills, weakenings and maps (`Ctx.KillMap.effLive`, `Ctx.accessible_killed`), and the names of a set under a capture weakening (`Ctx.mem_names_weakenC`, `Ctx.killFor_weakenC`) |
| `Typing` | the judgments `Γ ⊢ e : S ≤ T`, `Γ ⊢ᵉ g : E ≤ E'`, `Γ ⊢ φ : S ≡ T`, `Γ ⊢ h : x ∋ ℓ`, `Γ ⊢ m : src ⇒ Tel`, `Γ ⊢ₐ a : T`, `Γ ⊢ₚ p : E`, `Γ ⊢ t :ᵉ E`, `Γ ⊢ᵥ v : T`, `Γ ⊢ᵥᵉ v : E`, `Γ ⊢ᶠ[A] F` |
| `ModeBounds` | closed capture evidence keeps every mode bound (`Ctx.ModeSound`), by one mutual induction with the two `member` rules as hypotheses |
...
| `Preservation` | `preservation` (modulo the `FormsTyped` obligation) and `preservation'`, with the steps that substitute into a body |
| `Retyping` | the lemmas each step reads: store lookups, continuation weakening over the index of consumed leaves (`Cont.Typed.weakenC`, `Cont.Typed.heir`, `Cont.Typed.mono`), the frame clause under weakening, the substitution instances the machine uses, the answer sort, `FormsTyped` with `Ctx.NamesLe` exact at `consume`, and access and separation across a cover up to ownership (`Ctx.Accessible.of_namesLe`, `Ctx.ArgSep.of_cover`).  The five store lemmas of plan-5h S0.8 (`substAtom_store`, `enter_store`, `adjust_store`, `unpackF_store`, `letexBody_instC_store`) are not here: the first is false under decision 39 (`s0-g6s-counterexample.lean`) |
...
| `ModeExamples` | the mode and name facts of the separation line (M1 to M3 and the names of roots, heirs and capture names), decided in the kernel |
| `TermExamples` | cells, fresh packs and kills in the checker, W1: the programs of the weakening round, refused by the kill premise, by the premise of the arrow rule, or by the expansion of roots, and a closure that types under an heir, and O1: the programs of the ownership round, refused by the argument premise, the bound premise or the second conjunct of `ArgSep`, and the reducts that type at the ghost a step gives |
