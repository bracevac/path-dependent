...
`Coercions.Separation.FCdot.Checker` gives an executable kernel that returns the
...
Soundness lives in `Coercions.Separation.FCdot.Checker`; here it is paired with
