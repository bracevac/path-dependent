...
/-- Impredicative bottom, encoded as `forall X. X`. -/
def bottom : Ty sig :=
  .poly (.tvar .here)
...
| top : Ty sig -> Co sig
/-- Eliminate impredicative bottom into an arbitrary target type. -/
| bottom : Ty sig -> Co sig
/-- Compute a target term from the coerced source value. -/
| adapter : Ty sig -> Exp (sig ,, .var) -> Co sig
...
/-! A shared structural measure for recursion across coercions and expressions. -/

mutual

def Co.complexity : Co sig -> Nat
| .cvar _ | .refl _ | .top _ | .bottom _ => 1
| .trans first second | .arrow first second | .qual first second =>
    first.complexity + second.complexity + 1
| .adapter _ body => body.complexity + 1
| .poly body => body.complexity + 1

def Exp.complexity : Exp sig -> Nat
| .var _ => 1
| .abs _ body | .tabs body | .cabs _ _ body => body.complexity + 1
| .app function argument =>
    function.complexity + argument.complexity + 1
| .tapp function _ => function.complexity + 1
...
| .bottom T, rename => .bottom (T.rename rename)
| .adapter source body, rename =>
    .adapter (source.rename rename) (body.rename (rename.lift .var))
...
@[simp] theorem Ty.bottom_rename (rename : Rename sig sig') :
    (Ty.bottom : Ty sig).rename rename = Ty.bottom := rfl
