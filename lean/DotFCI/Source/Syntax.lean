...
`DotFCI` is the conservative, forward source language for Milestone 4.  It
keeps the heterogeneous scopes and variable-only stable paths of `DotFC`, but
adds intersections and objects containing several exact type definitions.
The older `DotFC.Source` language remains unchanged as the Milestone 0--3
baseline.
-/
