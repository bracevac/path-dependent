...
### 8.3 Plan-directed merging of aligned record spines
...
proceed. `LambdaPFCI` therefore adds an explicit, proof-relevant merge plan.
Write
...
when two aligned generalized-type views are to be combined into one. Here a
generalized type is either a proper type or an abstract-member interval. The
plan is generated by these four rules:
...
The auxiliary lower-bound plan has exactly two choices:

~~~text
───────────────── lower-same       ─────────────────── lower-union
Γ ⊢ L ⊔ L ⇒ L                  Γ ⊢ L₁ ⊔ L₂ ⇒ L₁ ∨ L₂
~~~

A completed proper-type plan induces subtyping:
...
The important premise in the pair rule is the second one: the member plan is
checked under the **merged** prefix `x:S`, not under either source prefix.
Consequently the plan can recurse through an entire binder-dependent record
spine. It still describes only aligned layouts: labels and member order must
match at every pair node. It performs no row permutation, width merge, or
label search.

The merge judgment describes how evidence is combined, not whether its output
is a valid source type. Target well-formedness remains a separate obligation
of term subsumption. In particular, after merging intervals one must still
prove `L <: U`; choosing a joined lower bound and a merged upper bound does not
establish that cross-bound automatically.

Semantically, compiling a plan in an environment closes over exactly the
information needed to execute it later:
...
There is one such semantic merge coercion and one plan interpreter. Given two
certificates for the same referent, the interpreter recurses structurally on
`M`. At a pair node, immutable lookup functionality identifies the same
physical pair cell and member referent in both views. It first merges the two
predecessor certificates, extends `ρ` with that concrete merged predecessor,
and only then interprets the dependent member plan. At an interval node, the
common stored witness receives the same or joined lower evidence and the
recursively merged upper evidence. Thus neither records nor types are combined
at runtime, and termination follows directly from the finite merge plan.

For example, the interval rule with lower-union and meet as its upper plan
recovers the familiar merge

~~~text
L₁..U₁ ⋈ L₂..U₂ ⇒ (L₁ ∨ L₂)..(U₁ ∧ U₂).
~~~

The common witness `W` supplies `L₁ ⇝ W` and `L₂ ⇝ W`, which union
elimination combines into `L₁ ∨ L₂ ⇝ W`; its two upper certificates are
retained together. This is a special case of the recursive judgment, not a
separate primitive rule.
...
The stronger record regression changes an earlier prefix and a later dependent
member in the same plan. Define:

~~~text
Q₀ = { A : S..S }       Qₗ = { A : S..L }
Qᵣ = { A : S..R }       Qᵢ = { A : S..(L ∧ R) }

P₀ = { x:Q₀; f:x.A }       Pₗ = { x:Qₗ; f:x.A }
Pᵣ = { x:Qᵣ; f:R }        Pᵢ = { x:Qᵢ; f:x.A ∧ R }
~~~

A physical spine built at `P₀` can be viewed at both `Pₗ` and `Pᵣ` because
`S <: L` and `S <: R`, and hence at `Pₗ ∧ Pᵣ`. Its merge plan recursively
establishes `Qₗ ⋈ Qᵣ ⇒ Qᵢ`, then checks the later plan
`x.A ⋈ R ⇒ x.A ∧ R` under `x:Qᵢ`. Thus both the prefix and the member
that depends on it change simultaneously:

~~~text
Pₗ ∧ Pᵣ <: Pᵢ.
~~~

A path-only alias `q:Pᵢ` selects `q.f : q₀.A ∧ R`, where `q₀` denotes its
stored prefix. The left certificate passes through the upper bound
`q₀.A <: L` for the operator; the right certificate supplies `R=F` for the
argument. Thus `q.f q.f` is typed and still executes one stored closure in one
stored record spine.
...
| Source subtyping | add six lattice rules and one plan-directed recursive aligned-merge rule |
| Realization | add paired meet and tagged union certificates |
| Subtyping interpretation | compile the lattice rules and the environment-closed merge plan |
| Coercion action | add lattice cases and one structural merge-plan interpreter |
...
`SemanticAction.lean`, and `SemanticWeakening.lean`. Its six dedicated
regressions cover ordinary function intersections, same-slot and aligned
record merging, recursive mixed record spines, shared-lower type members, and
general interval merging.
