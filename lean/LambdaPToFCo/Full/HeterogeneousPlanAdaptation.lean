...
inductive HeterogeneousPlanAdaptation
  ...
  | reflexiveUnderWiden
      {n : Nat} {context : Ctx n}
      {path : LambdaPFC.Path n} {referent : Ty n}
      (precise : Path.Ty context path (.ty referent))
      {producerScope : ScopeModel
        (context.snoc (.Single path)) targetContext}
      {demandScope : ScopeModel (context.snoc referent) targetContext}
      (alignment : ScopeAlignment producerScope.view demandScope.view)
      {endpointType : LambdaPFC.Ty (n + 1)}
      (producer : PositivePlan (context.snoc (.Single path)) targetContext
        producerScope.view endpointType)
      (demand : NegativePlan (context.snoc referent) targetContext
        demandScope.view endpointType)
      (commonPlan : ValuePlan sig)
      (producerPlan_eq : producer.plan = commonPlan)
      (demandPlan_eq : demand.plan = commonPlan) :
      @HeterogeneousPlanAdaptation sig targetContext (n + 1)
        (context.snoc (.Single path)) (context.snoc referent)
        (.snoc (.identity context) (.widen precise)) producerScope demandScope
        alignment endpointType endpointType .refl producer demand
  | toTop
...
noncomputable def adapter
  ...
  | reflexiveUnderWiden _ _ producer demand commonPlan producerPlan_eq
      demandPlan_eq =>
      rw [producerPlan_eq, demandPlan_eq]
      exact StableIdentity.Adapter.identity targetContext commonPlan
...
/-- Reflexive observed endpoints may cross exactly the heterogeneous binder
relation generated by widening a translated singleton path to its precise
referent.  Both endpoint plans must be tied to one explicit common plan, so
the eliminator computes identity without accepting a stable adapter or an
arbitrary-subtyping same-plan escape hatch. -/
noncomputable def underWidenReflexive
    {n : Nat} {context : Ctx n}
    {sig : Sig} {targetContext : SystemFCoExt.Ctx sig}
    {path : LambdaPFC.Path n} {referent : Ty n}
    (precise : Path.Ty context path (.ty referent))
    ...
    (producerPlan_eq : producer.plan = commonPlan)
    (demandPlan_eq : demand.plan = commonPlan) :
    HeterogeneousPlanAdaptation targetContext
      (.snoc (.identity context) (.widen precise)) alignment .refl producer
      demand :=
  .reflexiveUnderWiden precise alignment producer demand commonPlan
    producerPlan_eq demandPlan_eq
...
inductive ContravariantHeterogeneousPlanAdaptation
  ...
  | reflexiveUnderWiden
      ...
      {path : LambdaPFC.Path n} {referent : Ty n}
      (precise : Path.Ty context path (.ty referent))
      {producerScope : ScopeModel (context.snoc referent) targetContext}
      {demandScope : ScopeModel
        (context.snoc (.Single path)) targetContext}
      (alignment : ScopeAlignment producerScope.view demandScope.view)
      {endpointType : LambdaPFC.Ty (n + 1)}
      (producer : PositivePlan (context.snoc referent) targetContext
        producerScope.view endpointType)
      (demand : NegativePlan (context.snoc (.Single path)) targetContext
        demandScope.view endpointType)
      (commonPlan : ValuePlan sig)
      (producerPlan_eq : producer.plan = commonPlan)
      (demandPlan_eq : demand.plan = commonPlan) :
      @ContravariantHeterogeneousPlanAdaptation sig targetContext (n + 1)
        (context.snoc (.Single path)) (context.snoc referent)
        (.snoc (.identity context) (.widen precise)) producerScope demandScope
        alignment endpointType endpointType .refl producer demand
  | fromBottom
...
/-- Dual reflexive endpoint adaptation for the same exact widening-generated
binder relation.  The positive endpoint lives under the referent binder and
the negative endpoint under the singleton binder, as required by a lower
interval bound; both are fixed to one explicit plan and identity is computed
internally. -/
noncomputable def underWidenReflexive
    {n : Nat} {context : Ctx n}
    {sig : Sig} {targetContext : SystemFCoExt.Ctx sig}
