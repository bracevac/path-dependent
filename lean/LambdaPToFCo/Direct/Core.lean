...
This leaf connects source variables to raw target package expressions.  It
does not store target typing derivations in generated syntax, and it opens no
Church package eagerly.  The later path compiler is responsible for focused
elimination.
...
def WellTyped (targetContext : SystemFCo.Ctx sig)
    (slot : Slot sig) : Type :=
  Exp.HasType targetContext slot.expression slot.plan.inputTy
...
noncomputable def WellTyped.rename
  ...
  change Exp.HasType targetContext (slot.expression.rename mapping)
    (slot.plan.rename mapping).inputTy
  rw [← Package.Plan.inputTy_rename]
  exact SystemFCo.Exp.HasType.rename typedSlot typedMapping

end Slot

/-- A syntax-only correspondence from source variables to target package
...
/-- Reindex every target package in an environment. -/
...
/-- Add one raw target package variable.  Opening its hidden interface is a
...
def bind {n : Nat} {sourceContext : LambdaPFC.Ctx n}
  ...
  lookup := Fin.cases
    { plan := plan.rename (Rename.weaken .var)
      expression := .var .here }
...
@[simp] theorem bind_here
    ...
    (environment : Env sourceContext sig)
    (sourceType : LambdaPFC.Ty n) (plan : Package.Plan sig) :
    (environment.bind sourceType plan).lookup 0 =
      { plan := plan.rename (Rename.weaken .var)
...
@[simp] theorem bind_there
    ...
    (sourceType : LambdaPFC.Ty n) (plan : Package.Plan sig)
    (index : Fin n) :
    (environment.bind sourceType plan).lookup index.succ =
      (environment.lookup index).rename (Rename.weaken .var) := by
  rfl

/-- Every raw environment slot is checked at its plan's package input type. -/
...
/-- Binding a package plan preserves extrinsic environment typing. -/
noncomputable def WellTyped.bind
    ...
    (sourceType : LambdaPFC.Ty n) (plan : Package.Plan sig) :
    WellTyped (targetContext.bindVar plan.inputTy)
      (environment.bind sourceType plan) where
  lookup index := by
    refine Fin.cases ?_ (fun older => ?_) index
    · change Exp.HasType (targetContext.bindVar plan.inputTy) (.var .here)
        (plan.rename (Rename.weaken .var)).inputTy
      rw [← Package.Plan.inputTy_rename]
      exact .var Ctx.Lookup.here
    · exact (typedEnvironment.lookup older).rename
        (Rename.weaken .var)
        (Rename.Typed.weaken targetContext (.var plan.inputTy))
