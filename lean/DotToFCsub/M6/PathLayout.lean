...
M6 keeps allocation keyed by the syntactic pair `(path, label)`.  Path
equality never changes this table: every key owns a distinct private FCsub
name, while `PathEquality` constructs explicit equality evidence between
...
/-- The unquotiented source key used by M6 member allocation. -/
