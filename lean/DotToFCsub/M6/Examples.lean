...
import DotToFCsub.M5.Examples
import DotFCRP.Source.Examples

/-!
# M6 end-to-end regressions

The finite layout below allocates both members of the M5 two-member recursive
object at each of two syntactically different source paths:
...
namespace DotToFCsub.M6.Examples
...
abbrev Definitions := DotToFCsub.M5.Examples.Definitions

def encoding : DotToFCsub.M5.Encoding (target := []) Definitions :=
  DotToFCsub.M5.Examples.encoding

def firstMember : Fin Definitions.length :=
  DotToFCsub.M5.Examples.firstPosition

def secondMember : Fin Definitions.length :=
  DotToFCsub.M5.Examples.secondPosition
...
/-- Embed the closed M5 recursive object type below the `r` and `s` binders;
...
member of the concrete two-member M5 recursive interface. -/
...
selection derivation in the context that binds `t` to the embedded M5 object
...
/-- The complete concrete layout packages the original M5 recursive object
...
/-- Lower transport is directed from the exact M5 witness through the left
...
def closedAdministrativeTerm : FCsub.Tm [] :=
  DotToFCsub.M6.closePathEqualityRoundTrip singletonMemberEquality
...
theorem closed_administration_erases :
  ...
  simpa [closedAdministrativeTerm, layout, AliasScope.eraseAliases] using
    (DotToFCsub.M6.erase_closePathEqualityRoundTrip
