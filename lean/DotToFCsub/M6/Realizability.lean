...
import DotFCRP.Source.Runtime
import FCsub.CheckerCompleteness

/-!
# Realizability for traceable path aliases

Given a finite layout certificate, the M6 realization theorem combines three
...
/-- Every allocated source member is tied to one exact M5 member interface.
...
/-- Two realization witnesses for one syntactic key choose the same M5
...
/-- Co-resolved paths selecting the same label also agree on the exact M5
...
M5 witness, with the orientations checked by FCsub.  This structure does not
...
/-! ## Closing the M5 object below all generated aliases -/

/-- Wrap the M5 recursive package in one existing FCsub `newtype` per
...
/-- The complete alias allocation preserves the M5 recursive package type. -/
...
/-- Full realization of a supplied finite M6 layout over an M5 recursive
interface. -/
...
/-- Construct the complete typed, checker-accepted, runtime-reachable M6
object from the finite trace/member realization. -/
...
/-- Dynamic receivers are outside the traceable M6 translation boundary. -/
