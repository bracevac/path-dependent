...
member term expands according to `MemberEncoding`; a reusable Stage-A member
handle remains a proof-only alias and allocates no FCsub binder.
-/
...
/-- Target signature induced by a mixed Stage-A context. -/
...
/-- Readiness of a mixed Stage-A binding.  Member handles alias an existing
...
/-- Interface shapes admitted by the current direct-slot M3 boundary. -/
