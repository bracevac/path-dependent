import DotToFCsub.M4.StableFragment
import DotToFCsub.MemberEncoding

/-!
# Total M4 type and context translation
...
theorem shared_slot {sourceScope : DotFC.Sig}
  ...
  Layout.MemberLookup.functional first.lookup second.lookup
...
theorem shared_name {sourceScope : DotFC.Sig}
  ...
  congrArg Layout.MemberSlot.name (shared_slot first second)
...
theorem shared_bounds {sourceScope : DotFC.Sig}
  ...
  congrArg Layout.MemberSlot.bounds (shared_slot first second)
...
theorem shared_payload {sourceScope : DotFC.Sig}
  ...
  congrArg Layout.MemberSlot.payload (shared_slot first second)
...
/-- The M4 singleton package has the same exact scope as the M0--M3 member
convention: one name, two inclusions, then one payload. -/
...
theorem name_eq (targetScope : FCsub.Sig) (label : Name) :
    (Layout.MemberSlot.ofIndex targetScope label (index label)).name =
...
theorem payload_eq (targetScope : FCsub.Sig) (label : Name) :
    (Layout.MemberSlot.ofIndex targetScope label (index label)).payload =
      (DotToFCsub.MemberEncoding.payload :
        FCsub.BVar (DotToFCsub.MemberEncoding.Payload targetScope) .term) :=
  rfl

/-- M4 uses newest-first finite indices.  Thus the semantic lower position is
the legacy upper binder position; this is only an administrative evidence
permutation and does not affect the shared static name or payload. -/
theorem lower_position_eq_legacy_upper (targetScope : FCsub.Sig)
    (label : Name) :
    ((Layout.MemberSlot.ofIndex targetScope label (index label)).bounds.get
      ⟨0, by simp [Layout.MemberSlot.ofIndex, index]⟩).lower =
...
theorem upper_position_eq_legacy_lower (targetScope : FCsub.Sig)
    (label : Name) :
    ((Layout.MemberSlot.ofIndex targetScope label (index label)).bounds.get
      ⟨0, by simp [Layout.MemberSlot.ofIndex, index]⟩).upper =
