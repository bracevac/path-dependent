...
/-!
# Soundness of the M7 case-study compiler
...
which is exactly the existing closed recursive-object theorem from M5.
-/
...
/-- The M5 recursive package remains typed after weakening below every M6
alias pair. -/
...
/-- Closing all generated names/equalities restores the original M5 package
...
theorem erase_recursiveObjectInAliases {program : Surface.Program}
  ...
  simp [recursiveObjectInAliases,
    DotToFCsub.M5.erase_target_recursive_object,
...
/-- The expected closed runtime is insensitive to any number of static M6
alias pairs. -/
@[simp]
...
/-- All certified layers of the deliberately narrow M7 case study.
...
structure CertifiedResult {program : Surface.Program}
  ...
  sourceObject : M5.RecursivePreservation certificate.encoding
  signatureEndpoints :
    DotFCRP.Source.Wf program.aliases certificate.pathContext
        program.leftSelection ×
      DotFCRP.Source.Wf program.aliases certificate.pathContext
        program.rightSelection
  completeLayout : M6.AliasedRecursiveObjectRealization certificate.layout
    certificate.encoding FCsub.Ctx.nil
  singletonMember : M6.SingletonMemberRealization certificate.leftMember
