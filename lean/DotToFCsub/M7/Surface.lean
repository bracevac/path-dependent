import DotFCRP.Source.Typing
import DotToFCsub.M5.RecursiveLayout

/-!
# Shallow Scala-like surface for the compiler case study

M7 does not introduce another source calculus or another source type checker.
The declarations below are only names and smart constructors for the existing
`DotFCR` recursive-object syntax and `DotFCRP` traceable-path syntax.
...
alias store.  The proof-directed front end in `Elaboration` supplies the
existing source validity, path-resolution, and bridge certificates.
-/

namespace DotToFCsub.M7.Surface
...
def recursiveTypeMember (label : Name)
    (witness : RecursiveTy DotToFCsub.M5.ClosedSelfScope) :
    TypeDef DotToFCsub.M5.ClosedSelfScope :=
  ⟨label, witness⟩

/-! ## Proof-free surface program data -/

/-- The deliberately small M7 compilation unit.

`definitions` is the closed recursive object from M5.  `aliases`, `leftPath`,
and `rightPath` are ordinary M6 path data.  No typing or translation proof is
stored here. -/
structure Program where
  definitions : List (TypeDef DotToFCsub.M5.ClosedSelfScope)
  pathScope : Sig
...
/-- The existing M5 source term represented by a surface program. -/
